...
def check_tbs(symbol, direction, crl, crh):
    ...
    Bullish TBS:
      - TBS candle: body CLOSES below CRL (sweeps the low with body)
      - Next candle: CLOSES back above CRL (price returns into range)
      - Entry = OPEN of the TBS candle (first touch back inside range)
      - SL    = low of the TBS candle (the sweep extreme)

    Bearish TBS:
      - TBS candle: body CLOSES above CRH
      - Next candle: CLOSES back below CRH
      - Entry = OPEN of the TBS candle
      - SL    = high of the TBS candle
    ...
    for tf in TBS_TFS:
        ...
        for i in range(len(recent)-1):
            ...
            if direction=="BUY":
                # Body closes below CRL
                if c["close"] < crl and nxt["close"] > crl:
                    # Entry = open of TBS candle (Model #1)
                    # This is where price was before the sweep — the "return" level
                    tbs_entry = round(c["open"], 8)
                    tbs_sl    = round(c["low"],  8)   # SL at sweep extreme
                    return True, tf, tbs_entry, tbs_sl
            else:
                # Body closes above CRH
    ...
    for crt_tf in ["Hour4","Hour3","Hour2","Min60"]:
        ...
        for ob_tf in OB_TFS:
            ...
            for ob in obs[:4]:
                ...
                if not liq:
                    diag["no_liq"]+=1; continue

                tapping = price_tapping_ob(crt_candles, ob, direction)
                if not tapping:
                    ob_mid = (ob["top"] + ob["bot"]) / 2
                    last_close = crt_candles[-1]["close"] if crt_candles else 0
                    near_ob = abs(last_close - ob_mid) / ob_mid < 0.02 if ob_mid > 0 else False
                    if not near_ob:
                        diag["not_tapping"]+=1; continue

                crts = detect_crt(crt_candles, direction, ob=ob)
                if not crts:
                    diag["no_crts"]+=1; continue
...
<div class="tab-wrap">
  ...
  <div id="tab-log" style="display:none">
    <!-- GATE DIAGNOSTICS -->
    <div style="background:var(--ink2);border:1px solid var(--border);border-radius:2px;padding:16px 20px;margin-bottom:14px">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:.85rem;letter-spacing:.15em;color:var(--gold);margin-bottom:12px">Gate Diagnostics — Why Signals Are Rare</div>
      <div id="diag-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px"></div>
    </div>
    <div class="log-wrap">
...
async function fetchLog(){
  ...
  try{
    ...
    $('lbody').innerHTML=d.log.map(l=>{
      const cls=l.includes('🎯')?'ll-s':l.includes('❌')||l.includes('Error')?'ll-e':l.includes('GATE')||l.includes('STATS')?'ll-s':'ll-i';
  ...
  try{
    ...
    const labels={
      neutral:'Neutral Trend',not_continuous:'Weak Structure',
      no_obs:'No OBs Found',not_at_key:'OB Not At Key Level',
      not_in_zone:'OB Wrong Zone',no_liq:'No Liq Sweep',
      not_tapping:'Price Not At OB',no_crts:'No CRT Pattern',
      no_tbs:'No TBS Body Close',rr_low:'RR Below 2R',
      "1d_no_crts":'1D No CRT',"1d_no_tbs":'1D No TBS',"1d_rr_low":'1D RR Low',
      passed:'✅ SIGNALS PASSED'
    };
    const colors={
      neutral:'var(--dim)',not_continuous:'var(--dim)',
      no_obs:'var(--orange)',not_at_key:'var(--orange)',
      not_in_zone:'var(--orange)',no_liq:'var(--red)',
      not_tapping:'var(--red)',no_crts:'var(--red)',
      no_tbs:'var(--red)',rr_low:'var(--orange)',
      "1d_no_crts":'var(--dim)',"1d_no_tbs":'var(--dim)',"1d_rr_low":'var(--dim)',
      passed:'var(--green)'
    };
    const grid=$('diag-grid');
    if(grid){
      grid.innerHTML=Object.entries(d).map(([k,v])=>`
        <div style="background:var(--ink3);border:1px solid var(--muted);border-radius:2px;padding:10px 12px">
          <div style="font-family:'JetBrains Mono',monospace;font-size:.52rem;color:var(--dim);letter-spacing:.06em;margin-bottom:5px;text-transform:uppercase">${labels[k]||k}</div>
          <div style="font-family:'Bebas Neue',sans-serif;font-size:1.5rem;color:${colors[k]||'var(--text)'}">${v}</div>
        </div>`).join('');
    }
  }catch{}
}
...
@app.route("/api/diag")
def api_diag():
    """Return gate diagnostic counters — shows why signals are rare."""
...
# ═════════════════════════════════════════════════════════════════════
# ADVANCED AUTO-TRADING EXTENSIONS
# Added based on user requested modifications
# ═════════════════════════════════════════════════════════════════════
...
MAX_LIVE_TRADES      = 4
BALANCE_USAGE_LIMIT  = 0.30
MARGIN_PER_TRADE     = 0.25
MAX_SL_MARGIN_LOSS   = 0.70

# --- LIVE TRADE STORAGE ---
running_trades  = []
completed_trades = deque(maxlen=10)
monitored_setups = []
...
def mapped_tbs_tfs(crt_tf):
    return CRT_LTF_MAPPING.get(crt_tf, ["Min5"])

# --- BREAKER BLOCKS ---
def find_breaker_blocks(candles, direction):
    blocks = []
    if len(candles) < 10:
        return blocks
...
# --- REJECTION BLOCKS ---
def find_rejection_blocks(candles, direction):
    out = []
...
def detect_manipulation_phase(candles, direction):
    if len(candles) < 20:
        return False
    ...
    if direction == "BUY":
        ...
        return min(lows) < min([x["low"] for x in candles[-20:-8]])

    highs = [x["high"] for x in recent]
    return max(highs) > max([x["high"] for x in candles[-20:-8]])

# --- INVALIDATION LOGIC ---
def invalid_sweep_close(c1, c2, direction):
    if direction == "BUY":
        return c2["close"] < c1["low"]
    return c2["close"] > c1["high"]

# --- LEVERAGE ENGINE ---
def auto_select_leverage(entry, sl, balance, a_plus=False):
    risk_pct = abs(entry - sl) / entry

    if risk_pct <= 0:
        return 10

    lev = int(min(max((MAX_SL_MARGIN_LOSS / risk_pct), 10), 500))

    if a_plus:
        lev = min(500, int(lev * 1.2))

    return lev

# --- POSITION SIZE ENGINE ---
def calculate_margin(balance):
    return round(balance * MARGIN_PER_TRADE, 2)

# --- PLACE ORDER PLACEHOLDER ---
def place_mexc_futures_trade(symbol, direction, entry, sl, tp, leverage):
    '''
    PLACEHOLDER:
    User requested automatic MEXC trading.
    Connect this to signed MEXC private endpoints.

    Required:
    - HMAC SHA256 signing
    - Timestamp auth
    - POST order endpoints
    - Reduce-only TP/SL logic
    '''
...
# --- MONITORING ENGINE ---
def monitor_manipulation_setup(symbol, tf, key_level_type, direction):
    if len(monitored_setups) >= MAX_MONITORED_SETUPS:
        return False

    monitored_setups.append({
        "symbol": symbol,
        "tf": tf,
...
# --- RECENT TRADE STORAGE ---
def close_trade(trade, pnl, roi):
    trade["pnl"] = pnl
    trade["roi"] = roi
    trade["status"] = "CLOSED"

    completed_trades.appendleft(trade)

    if trade in running_trades:
        running_trades.remove(trade)

# --- DASHBOARD HTML INJECTION HELPERS ---
DASHBOARD_EXTRA_SECTIONS = '''
...
# ═════════════════════════════════════════════════════════════════════
# REAL MEXC FUTURES EXECUTION ENGINE
# ═════════════════════════════════════════════════════════════════════
...
def mexc_private_request(method, endpoint, params=None):
    if params is None:
        params = {}
    ...
    sign = mexc_sign(params, MEXC_API_SECRET)
    params["sign"] = sign
...
def get_futures_balance():
    ...
    r = mexc_private_request("GET", endpoint)

    try:
        for asset in r.get("data", []):
...
def set_leverage(symbol, leverage):
    ...
    return mexc_private_request("POST", endpoint, params)

def place_market_order(symbol, side, volume, leverage):
...
def place_tp_sl(symbol, tp, sl, position_id):
...
def calculate_position_size(balance, entry, leverage):
    usable_margin = balance * MARGIN_PER_TRADE
...
def execute_auto_trade(
    ...
    if len(running_trades) >= MAX_LIVE_TRADES:
        return {
            "success": False,
            "error": "MAX LIVE TRADES REACHED"
        }
...
def auto_execute_valid_setup(signal):
    try:
        execute_auto_trade(
            ...
            sl=signal["sl"],
            tp=signal["tp"],
            a_plus=signal.get("grade") == "A+"
        )

    except Exception as e:
        print("AUTO EXECUTION ERROR:", e)
