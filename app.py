...
def find_fvg(symbol, tf, direction):
    """
    Detect FVG (Fair Value Gap) or IFVG (Inverse FVG) on the LTF.

    Entry is placed at the TIP of the FVG — the exact edge price
    touches first when it comes to mitigate the gap.

    Bullish FVG (fresh, unmitigated):
        Gap = C1.high to C3.low (C3.low > C1.high)
        Entry TIP = C1.high  ← first level price touches entering gap from below

    Bearish FVG (fresh, unmitigated):
        Gap = C3.high to C1.low (C3.high < C1.low)
        Entry TIP = C1.low   ← first level price touches entering gap from above

    IFVG Bullish (old bullish FVG that got filled → now acts as resistance):
        Entry TIP = zone_top (C3.low of original gap) ← price touches from above

    IFVG Bearish (old bearish FVG that got filled → now acts as support):
        Entry TIP = zone_bot (C3.high of original gap) ← price touches from below

    Priority: fresh FVG > IFVG
    Returns: (found, fvg_type, entry_tip, zone_top, zone_bot)
    """
    candles = get_candles(symbol, tf, limit=60)
    ...
    for i in range(len(candles) - 3):
        ...
        if direction == "BUY":
            ...
            if c3["low"] > c1["high"]:
                zone_bot  = c1["high"]   # tip = bottom of gap (first touch from below)
                zone_top  = c3["low"]
                entry_tip = round(zone_bot, 8)

                # Check if any later candle has traded INTO the gap
                mitigated = any(
                    candles[j]["low"] <= zone_top
                    for j in range(i + 3, len(candles))
                )

                if not mitigated:
                    # Fresh FVG — entry at tip (C1.high)
                    fresh_fvgs.append({
                        "type":      "FVG",
                        "entry":     entry_tip,
                    ...
                    # Filled FVG → IFVG now acts as resistance
                    # Entry tip = top of old gap (C3.low) — price touches from above
                    ifvg_zones.append({
                        "type":      "IFVG",
                        "entry":     round(zone_top, 8),
                        "zone_top":  round(zone_top, 8),
                        "zone_bot":  round(zone_bot, 8),
                        "idx":       i,
                    })
            ...
            if c3["high"] < c1["low"]:
                zone_top  = c1["low"]    # tip = top of gap (first touch from above)
                zone_bot  = c3["high"]
                entry_tip = round(zone_top, 8)

                mitigated = any(
                    candles[j]["high"] >= zone_bot
                    for j in range(i + 3, len(candles))
                )

                if not mitigated:
                    # Fresh FVG — entry at tip (C1.low)
                    fresh_fvgs.append({
                        "type":      "FVG",
                        "entry":     entry_tip,
                        "zone_top":  round(zone_top, 8),
                    ...
                    # Filled FVG → IFVG now acts as support
                    # Entry tip = bottom of old gap (C3.high) — price touches from below
                    ifvg_zones.append({
                        "type":      "IFVG",
                        "entry":     round(zone_bot, 8),
                        "zone_top":  round(zone_top, 8),
                        "zone_bot":  round(zone_bot, 8),
                        "idx":       i,
                    })

    # Return most recent fresh FVG first, fall back to most recent IFVG
    if fresh_fvgs:
        best = max(fresh_fvgs, key=lambda x: x["idx"])
        return True, best["type"], best["entry"], best["zone_top"], best["zone_bot"]
    if ifvg_zones:
        best = max(ifvg_zones, key=lambda x: x["idx"])
        return True, best["type"], best["entry"], best["zone_top"], best["zone_bot"]
...
def check_choch(symbol, tf, direction):
    """
    Detect Change of Character (CHOCH) on the lower timeframe.

    After a CRT sweep, CHOCH is the FIRST structural break that confirms
    the market has shifted delivery direction:

    Bullish CHOCH:
      - Find the most recent swing HIGH formed after the CRL sweep
      - A candle's body (close) breaks and closes ABOVE that swing high
      - Entry = that swing high level (the broken structure level)

    Bearish CHOCH:
      - Find the most recent swing LOW formed after the CRH sweep
      - A candle's body (close) breaks and closes BELOW that swing low
      - Entry = that swing low level (the broken structure level)

    Returns (found: bool, choch_level: float or None)
    choch_level = the exact structural level that was broken = entry price
    """
    ...
    if direction == "BUY":
        # Step 1: find swing highs (local highs) in the recent data
        # A swing high = candle whose high is higher than 2 neighbours on each side
        swing_highs = []
        for i in range(2, len(recent) - 2):
            c = recent[i]
            if (c["high"] > recent[i-1]["high"] and
                c["high"] > recent[i-2]["high"] and
                c["high"] > recent[i+1]["high"] and
                c["high"] >= recent[i+2]["high"]):
                swing_highs.append((i, c["high"]))

        if not swing_highs:
            # Fallback: use simple prev candle high if no swings found
            for i in range(len(recent) - 1, 1, -1):
                c    = recent[i]
                prev = recent[i - 1]
                if c["close"] > prev["high"] and c["close"] > c["open"]:
                    return True, round(prev["high"], 8)
            return False, None

        # Step 2: find the most recent swing high
        last_swing_idx, last_swing_high = swing_highs[-1]

        # Step 3: look for a candle AFTER the swing that closes above it (CHOCH)
        for i in range(last_swing_idx + 1, len(recent)):
            c = recent[i]
            # Body close must be above the swing high (not just a wick)
            if c["close"] > last_swing_high and c["close"] > c["open"]:
                return True, round(last_swing_high, 8)   # entry = broken swing high

    else:  # SELL
        # Step 1: find swing lows
        swing_lows = []
        for i in range(2, len(recent) - 2):
            c = recent[i]
            if (c["low"] < recent[i-1]["low"] and
                c["low"] < recent[i-2]["low"] and
                c["low"] < recent[i+1]["low"] and
        ...
        if not swing_lows:
            ...
            for i in range(len(recent) - 1, 1, -1):
                ...
                if c["close"] < prev["low"] and c["close"] < c["open"]:
                    return True, round(prev["low"], 8)
            return False, None

        # Step 2: most recent swing low
        last_swing_idx, last_swing_low = swing_lows[-1]

        # Step 3: candle after swing that closes below it
        for i in range(last_swing_idx + 1, len(recent)):
            c = recent[i]
            if c["close"] < last_swing_low and c["close"] < c["open"]:
                return True, round(last_swing_low, 8)   # entry = broken swing low
...
def score_signal(crt, trend, tbs_found, tbs_tf, tbs_strength, fvg_found, choch_found, fvg_type=None):
    score   = 0
    details = []
    ...
    # 6. FVG / IFVG entry (10 pts)
    if fvg_found:
        score += 10; details.append(f"✅ {fvg_type if fvg_type else 'FVG'} entry tip found (+10)")
    else:
        details.append("⚠️ No FVG (+0)")

    # 7. CHOCH confirmation (10 pts)
    if choch_found:
        score += 10; details.append("✅ CHOCH confirmed (+10)")
    else:
        details.append("⚠️ No CHOCH yet (+0)")
...
def format_signal_message(sig):
    ...
    fvg    = f"✅ {sig.get('fvg_type','FVG')} @ {sig.get('fvg_entry','–')} (top:{sig.get('fvg_top','–')} bot:{sig.get('fvg_bot','–')})" if sig.get("fvg_found") else "❌ Not found"
    choch  = "✅ Confirmed" if sig.get("choch_found") else "❌ Not confirmed"
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>Entry:</b>     {sig['entry']} <i>({sig.get('entry_type','CHOCH')})</i>\n"
        f"<b>SL:</b>        {sig['sl']}\n"
        f"<b>TP:</b>        {sig['tp']}\n"
        f"<b>RR:</b>        {sig['rr']}R\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def scan_pair(symbol):
    ...
    for tf in TFS:
        ...
        for crt in crts:
            ...
            # Lower TF for FVG/IFVG and CHOCH
            lower_tf = "Min60" if tf == "Day1" else "Min15"
            fvg_found, fvg_type, fvg_entry, fvg_top, fvg_bot = find_fvg(symbol, lower_tf, direction)
            choch_found, choch_level                           = check_choch(symbol, lower_tf, direction)

            # ── Entry Logic (priority order) ──────────────────────
            # 1st choice: FVG or IFVG tip on LTF (after CHOCH confirms)
            # 2nd choice: CHOCH level (structural break)
            # 3rd choice: C2 close (raw CRT entry, weakest)
            if fvg_found and fvg_entry and choch_found:
                entry      = fvg_entry
                entry_type = fvg_type          # "FVG" or "IFVG"
            elif fvg_found and fvg_entry:
                entry      = fvg_entry
                entry_type = f"{fvg_type} (no CHOCH)"
            elif choch_found and choch_level:
                entry      = choch_level
                entry_type = "CHOCH"
            else:
                entry      = crt["entry"]      # C2 close fallback
                entry_type = "C2 Close"

            # Recalculate RR from the actual entry
            sl = crt["sl"]
            tp = crt["tp"]
            risk   = abs(entry - sl)
            reward = abs(tp - entry)
            ...
            # Skip if RR drops below 2 after CHOCH entry adjustment
            if rr < 2.0:
                continue

            # Update crt dict with new entry/rr for scoring
            crt_scored = dict(crt)
            crt_scored["entry"] = entry
            crt_scored["rr"]    = rr

            score, grade, details = score_signal(
                crt_scored, trend, tbs_found, tbs_tf, tbs_strength, fvg_found, choch_found, fvg_type
            )
...
(function(){
  ...
  function buildCard(s,idx){
    ...
    const fvgLabel = s.fvg_found ? `✅ ${s.fvg_type||'FVG'}` : '❌ FVG';
    const fvg=s.fvg_found?`<span class="confirm-pill cp-ok">${fvgLabel}</span>`:`<span class="confirm-pill cp-no">❌ FVG</span>`;
    const choch=s.choch_found?`<span class="confirm-pill cp-ok">✅ CHOCH</span>`:`<span class="confirm-pill cp-no">❌ CHOCH</span>`;
    ...
    return `<div class="sig-card ${dir.toLowerCase()}">
      ...
      <div class="sig-levels">
        <div class="level"><div class="level-lbl">Entry (${s.entry_type||'FVG'})</div><div class="level-val entry">${fmt(s.entry)}</div></div>
        ...
        <div class="level"><div class="level-lbl">FVG Top</div><div class="level-val" style="color:#f59e0b">${fmt(s.fvg_top)}</div></div>
        <div class="level"><div class="level-lbl">FVG Bot</div><div class="level-val" style="color:#f59e0b">${fmt(s.fvg_bot)}</div></div>
      ...
      <div class="sig-confirms">${tbs}${fvg}${choch}</div>
