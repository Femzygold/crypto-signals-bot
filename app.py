...
TOP_PAIRS = ["BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT","DOGE_USDT","AVAX_USDT","LINK_USDT","TON_USDT","NOT_USDT","ONE_USDT","PEPE_USDT"]
...
def get_account_balance():
    ...
    """
    # Try the correct MEXC Futures endpoint first
    for endpoint in ["/account/asset", "/account/assets"]:
        ...
        if isinstance(data, dict):
            ...
            if currency == "USDT" or not currency:
                ...
                if bal >= 0:
                    return round(bal, 4), None
    return 0.0, err or "Balance not found — check API keys and futures account"
...
.prices-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:8px}
...
@media(max-width:820px){.stats-grid{grid-template-columns:1fr 1fr 1fr}.prices-grid{grid-template-columns:repeat(4,1fr)}.hdr-in,.sec,.tab-wrap{padding:0 13px}.prog{padding:7px 13px}.snum{display:none}.lvl-grid{grid-template-columns:1fr 1fr}.trade-form{grid-template-columns:1fr}.pos-grid{grid-template-columns:1fr}}
@media(max-width:480px){.stats-grid{grid-template-columns:1fr 1fr}.prices-grid{grid-template-columns:repeat(3,1fr)}}
</style>
</head>
<body>
...
<div class="sec">
  ...
  <div class="prices-grid" id="pgrid"><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div><div class="pc" style="min-height:70px"></div></div>
</div>
...
const TOP=["BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT","DOGE_USDT","AVAX_USDT","LINK_USDT","TON_USDT","NOT_USDT","ONE_USDT","PEPE_USDT"];
...
window.fetchBalance=async function(){try{const r=await fetch("/api/balance");const d=await r.json();if(d.error&&d.balance===0){showTradeMsg("❌ "+d.error,false);$("bal-val").textContent="–";}else{$("bal-val").textContent=Number(d.balance).toFixed(2);showTradeMsg("✅ Balance: $"+Number(d.balance).toFixed(2)+" USDT",true);}}catch{showTradeMsg("❌ Network error — check connection",false);}};
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState()];if(tick%2===0)ps.push(fetchPrices());if(activeTab==="log")ps.push(fetchLog());if(activeTab==="log"&&tick%3===0)ps.push(fetchDiag());if(activeTab==="monitor"&&tick%2===0)ps.push(fetchMonitor());if(activeTab==="trades")ps.push(window.fetchPnl());if(activeTab==="history"&&tick%5===0)ps.push(fetchHistory());if(activeTab==="paper"&&tick%2===0)ps.push(fetchPaperData());await Promise.all(ps);setTimeout(poll,2000);}
