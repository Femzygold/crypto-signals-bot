...
CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "trade_config.json")
...
def load_trade_config():
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            ...
            if MEXC_API_KEY:    merged["api_key"]    = MEXC_API_KEY
            if MEXC_API_SECRET: merged["api_secret"] = MEXC_API_SECRET
            return merged
    except Exception as e:
        print(f"[config] Failed to load saved config: {e}", flush=True)
    return defaults

def save_trade_config():
    try:
...
def place_order(sig):
    ...
    if err:
        log(f"❌ Auto-trade BLOCKED — balance error: {err}")
        return False, f"Balance error: {err}"
    if balance < 0.2:
        log(f"❌ Auto-trade BLOCKED — balance ${balance:.4f} below minimum $0.20")
        return False, f"Insufficient balance ${balance:.4f} (min $0.20)"

    total_used = len(open_trades) * (balance * 0.20)
    if total_used >= balance * 0.80:
        log(f"❌ Auto-trade BLOCKED — 80% balance cap reached (used={total_used:.2f} of {balance:.2f})")
    ...
    if size_raw < info["min_vol"]:
        log(f"⚠️ Calculated size {size_raw:.4f} < min_vol {info['min_vol']} — clamping to min_vol. "
            f"Balance may be too small for this pair's minimum contract.")
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                ...
                if trade_config["enabled"] and trade_config["api_key"] and trade_config.get("api_secret"):
                    ok, msg = place_order(sig)
                    log(f"{'✅' if ok else '❌'} Auto-trade: {msg}")
                elif trade_config["enabled"]:
                    missing = []
                    if not trade_config["api_key"]: missing.append("api_key")
                    if not trade_config.get("api_secret"): missing.append("api_secret")
                    log(f"❌ Monitor auto-trade BLOCKED — missing: {', '.join(missing)}")
                ...
                try:
                    ...
                    for sig in res:
                        ...
                        if trade_config["enabled"] and trade_config["api_key"] and trade_config.get("api_secret"):
                            ...
                            if not trade_config["api_key"]: missing.append("api_key")
                            if not trade_config.get("api_secret"): missing.append("api_secret")
                            log(f"❌ Auto-trade BLOCKED — missing: {', '.join(missing)}")
                        if paper_config["enabled"] and paper_config["auto_trade"]:
                            ok2, msg2 = place_paper_order(sig)
                            if ok2: log(f"📝 Paper auto: {msg2}")
                except Exception as e:
...
def api_trade_config():
    if request.method == "POST":
        ...
        save_trade_config()
        log(f"⚙️ Trade config updated & saved. Auto-trade: {'ON' if trade_config['enabled'] else 'OFF'} | "
            f"key={'SET' if trade_config['api_key'] else 'MISSING'} | "
            f"secret={'SET' if trade_config['api_secret'] else 'MISSING'}")
...
def start_scanner():
    ...
    log(f"🔧 Trade config at start: enabled={trade_config['enabled']} "
        f"api_key={'SET' if trade_config['api_key'] else 'MISSING'} "
        f"api_secret={'SET' if trade_config['api_secret'] else 'MISSING'} "
        f"risk_pct={trade_config['risk_pct']}%")
