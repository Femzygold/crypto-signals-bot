...
def load_trade_config():
    """Load persisted API keys and trade settings from disk."""
    defaults = {
        "enabled":    False,
...
def save_trade_config():
    """Persist trade_config (including API keys) to disk."""
    try:
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();if(d.api_key)$("t-apikey").value=d.api_key;$("t-risk").value=d.risk_pct||1;updateTradeBadge(d.enabled);const sf=$("t-secret");if(d.secret_saved){sf.placeholder="Secret saved ✔ (leave blank to keep)";sf.value="";}else{sf.placeholder="Your secret key";}}catch{}}
...
def api_trade_config():
    if request.method == "POST":
        ...
        with trade_lock:
            ...
            # Only overwrite secret if a non-empty value is actually sent
    ...
    # GET: never send secret back, just indicate whether one is saved
    cfg = {k: v for k, v in trade_config.items() if k != "api_secret"}
    cfg["secret_saved"] = bool(trade_config.get("api_secret", ""))
    return jsonify(cfg)
