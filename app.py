    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r") as f:
                saved = json.load(f)
            # Merge saved values over defaults (env vars win if saved is empty)
...
def save_trade_config():
    """Persist current trade_config (including API keys) to disk."""
    try:
...
def api_trade_config():
    if request.method == "POST":
        ...
        save_trade_config()   # ← persist to disk so keys survive restarts
