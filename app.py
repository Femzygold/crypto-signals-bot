...
MEXC_API_SECRET = os.environ.get("MEXC_API_SECRET_KEY", "e13578211318499baa3852677365d3cb")
...
_settings_path = os.path.join(_app_dir, "settings_saved.json")
if not os.access(_app_dir, os.W_OK):
    _settings_path = "/tmp/settings_saved.json"
    ...
    try:
        with open(SETTINGS_FILE, "w") as f:
            json.dump({"scan": dict(scan_settings),
                       "trade": {k:v for k,v in trade_config.items() if k!="enabled"}}, f)
...
trade_config = {
    "enabled":       False,   # never auto-enable on restart for safety
...
def fmt_tg_m2(sig):
    e    = "🟢" if sig["direction"]=="BUY" else "🔴"
    bars = "█"*(sig["score"]//10)+"░"*(10-sig["score"]//10)
    htf_label = {"Day1":"1D","Hour4":"4H","Hour3":"3H","Hour2":"2H","Min60":"1H"}.get(sig.get("tf",""),"–")
    ltf_label = sig.get("ob_tf","–")
    return (
        f"{e} <b>MAD MAN MODEL #2 — {sig['direction']}</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>Pair:</b>        {sig['symbol']}\n"
        f"<b>HTF Zone:</b>    {htf_label} · {sig.get('ob_zone','–')}\n"
        f"<b>LTF Entry:</b>   {ltf_label} · FVG Tip (1st touch)\n"
        f"<b>Trend:</b>       {sig['trend']}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>🎯 Entry:</b>    {sig['entry']}\n"
        f"<b>   Type:</b>     FVG Tip — first touch of gap\n"
        f"<b>🛑 SL:</b>       {sig['sl']} (sweep extreme + buffer)\n"
        f"<b>🎯 TP1:</b>      {sig.get('tp1','–')} (50% of range)\n"
        f"<b>🎯 TP2:</b>      {sig.get('tp2','–')} (other side / liquidity)\n"
        f"<b>📊 RR:</b>       {sig['rr']}R (to TP2)\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>Sweep High:</b>  {sig.get('sweep_extreme','–')}\n"
        f"<b>KL Zone:</b>     {sig.get('crh','–')} – {sig.get('crl','–')}\n"
...
def log(msg):
    ...
    with log_lock: scan_state["log"].appendleft(line)
...
# ════════ MAD MAN MODEL #2 ════════════════════════════════════════════
# Logic:
#  1. Price taps an HTF key level (OB / BB / FVG) — no CRT needed
#  2. Drop to LTF (30m / 15m / 5m)
#  3. Wait for price to sweep the HIGH of the first touch of that level
#  4. After the sweep, look for a bearish/bullish FVG (displacement)
#  5. Entry = FVG midpoint  (between fvg_top and fvg_bot)
#  6. SL    = beyond the sweep extreme (high+buffer for SELL, low-buffer for BUY)
#  7. TP1   = 50% of the range  (from sweep high → pre-sweep swing low)
#  8. TP2   = other side of the range (the swing low / liquidity)
#  9. Once 70% of TP2 range is reached → trail SL to TP1 level
#
# "First touch high" == the highest point reached when price first entered the KL zone

M2_LTF_ORDER = ["Min30", "Min15", "Min5", "Min3"]   # LTFs to scan in order

def _find_first_touch_extreme(candles, zone_top, zone_bot, direction, lookback=60):
    """
    Find the extreme price of the FIRST candle that entered the KL zone.
    For SELL (bearish KL): first touch high = highest high among candles that entered from below.
    For BUY  (bullish KL): first touch low  = lowest low among candles that entered from above.
    Returns (extreme_price, touch_idx) or (None, None).
    """
    recent = candles[-lookback:]
    for i, c in enumerate(recent):
        if direction == "SELL":
            # Price enters bearish zone from below
            if c["high"] >= zone_bot and c["low"] < zone_top:
                return c["high"], i
        else:
            # Price enters bullish zone from above
            if c["low"] <= zone_top and c["high"] > zone_bot:
                return c["low"], i
    return None, None

def _sweep_of_first_touch(candles, first_extreme, direction, touch_idx, lookback=40):
    """
    After the first touch, confirm a reaction then look for the sweep.

    SELL setup:
      1. After first touch, price must DUMP and make a lower low (reaction down)
         — confirms the zone is real and early buyers got trapped
      2. Then price pumps back up and sweeps ABOVE the first touch high
         — liquidity grab on the trapped sellers / early shorts

    BUY setup:
      1. After first touch, price must PUMP and make a higher high (reaction up)
         — confirms the zone is real and early sellers got trapped
      2. Then price dumps back down and sweeps BELOW the first touch low
         — liquidity grab on the trapped buyers / early longs

    Returns (sweep_candle, sweep_idx) or (None, None).
    """
    search = candles[touch_idx+1 : touch_idx+1+lookback]
    if not search:
        return None, None

    if direction == "SELL":
        # Step 1: find the reaction low after first touch (price dumps)
        reaction_low = None
        reaction_idx = None
        for j, c in enumerate(search):
            if reaction_low is None or c["low"] < reaction_low:
                reaction_low = c["low"]
                reaction_idx = j
        # Must be a meaningful dump — at least 0.1% below first touch high
        if reaction_low is None or reaction_low >= first_extreme * 0.999:
            return None, None
        # Step 2: after the reaction low, look for the sweep above first_extreme
        for j in range(reaction_idx + 1, len(search)):
            c = search[j]
            if c["high"] > first_extreme:
                return c, touch_idx + 1 + j

    else:  # BUY
        # Step 1: find the reaction high after first touch (price pumps)
        reaction_high = None
        reaction_idx  = None
        for j, c in enumerate(search):
            if reaction_high is None or c["high"] > reaction_high:
                reaction_high = c["high"]
                reaction_idx  = j
        # Must be a meaningful pump — at least 0.1% above first touch low
        if reaction_high is None or reaction_high <= first_extreme * 1.001:
            return None, None
        # Step 2: after the reaction high, look for the sweep below first_extreme
        for j in range(reaction_idx + 1, len(search)):
            c = search[j]
            if c["low"] < first_extreme:
                return c, touch_idx + 1 + j

    return None, None

def _fvg_after_sweep(candles, sweep_idx, direction, lookback=20):
    """
    After the sweep candle, find a fresh FVG showing displacement in
    the correct direction.
    SELL: displacement = bearish FVG  (c3.high < c1.low)
    BUY:  displacement = bullish FVG  (c3.low  > c1.high)
    Returns (fvg_top, fvg_bot, fvg_mid) or (None, None, None).
    """
    search = candles[sweep_idx : sweep_idx + lookback]
    for i in range(len(search) - 2):
        c1, c3 = search[i], search[i+2]
        if direction == "SELL":
            if c3["high"] < c1["low"]:
                fvg_top = c1["low"]; fvg_bot = c3["high"]
                fvg_tip = fvg_bot   # SELL: entry at BOTTOM of FVG (first price touches from above)
                return fvg_top, fvg_bot, fvg_tip
        else:
            if c3["low"] > c1["high"]:
                fvg_top = c3["low"]; fvg_bot = c1["high"]
                fvg_tip = fvg_top   # BUY: entry at TOP of FVG (first price touches from below)
                return fvg_top, fvg_bot, fvg_tip
    return None, None, None

def _swing_before_sweep(candles, touch_idx, direction, lookback=60):
    """
    Find the swing low (BUY) or swing high (SELL) BEFORE the first touch —
    this is the other side of the range / liquidity pool that becomes TP2.
    """
    search = candles[max(0, touch_idx-lookback) : touch_idx+1]
    if direction == "SELL":
...
def scan_pair_model2(symbol):
    """
    Mad Man Model #2 scanner.
    Returns list of signal dicts (same schema as Model #1 but model="2").
    """
    results = []

    # ── 1. HTF trend + key level ───────────────────────────────────────
    ref_candles = get_candles(symbol, "Hour4", limit=200)
    if not ref_candles or len(ref_candles) < 30:
        return results
    ...
    # Use Hour4 and Hour1 as the HTF key-level timeframes
    for htf in ["Hour4", "Min60"]:
        htf_candles = get_candles(symbol, htf, limit=150)
        if not htf_candles or len(htf_candles) < 20:
            continue

        # Build HTF key levels (same as Model 1)
        raw_obs = find_obs(htf_candles, "BULLISH" if direction=="BUY" else "BEARISH")
        valid_kls = []
        for ob in raw_obs[:6]:
            ob["kl_type"] = "OB"; valid_kls.append(ob)
        for bb in find_breaker_block(htf_candles, direction)[:3]:
            valid_kls.append(bb)
        # FVGs as KLs
        for i in range(max(0, len(htf_candles)-50), len(htf_candles)-3):
            c1x = htf_candles[i]; c3x = htf_candles[i+2]
        ...
        for kl in valid_kls[:10]:
            zone_top = kl.get("top", kl.get("high", 0))
            zone_bot = kl.get("bot", kl.get("low",  0))
            if zone_top <= zone_bot: continue

            # P/D zone check (same as Model 1)
            in_pd, pd_name = ob_in_pd_zone(kl, htf_candles, direction)
            if not in_pd: continue

            # ── 2. Drop to LTF ─────────────────────────────────────────
            for ltf in M2_LTF_ORDER:
                ltf_candles = get_candles(symbol, ltf, limit=200)
                if not ltf_candles or len(ltf_candles) < 20:
                    continue

                # ── 3. Find first touch of the zone ────────────────────
                first_extreme, touch_idx = _find_first_touch_extreme(
                    ltf_candles, zone_top, zone_bot, direction)
                if first_extreme is None:
                    continue

                # ── 4. Find sweep of first-touch extreme ───────────────
                sweep_candle, sweep_idx = _sweep_of_first_touch(
                    ltf_candles, first_extreme, direction, touch_idx)
                if sweep_candle is None:
                    continue

                # ── 5. Find FVG after sweep (displacement) ─────────────
                fvg_top, fvg_bot, fvg_mid = _fvg_after_sweep(
                    ltf_candles, sweep_idx, direction)
                if fvg_top is None:
                    continue

                # ── 6. Entry & SL ──────────────────────────────────────
                entry = round(fvg_mid, 8)
                buf   = (zone_top - zone_bot) * 0.05   # 5% of zone as buffer
                if direction == "SELL":
                    sl_p = round(sweep_candle["high"] + buf, 8)
                else:
                    sl_p = round(sweep_candle["low"]  - buf, 8)

                # ── 7 & 8. TP1 (50%) and TP2 (other side of range) ────
                liq_pool = _swing_before_sweep(ltf_candles, touch_idx, direction)
                if liq_pool is None:
                    continue

                if direction == "SELL":
                    # Range: sweep_high → liq_pool (the swing low before sweep)
                    sweep_high = sweep_candle["high"]
                    range_size = abs(sweep_high - liq_pool)
                    tp1 = round(sweep_high - range_size * 0.50, 8)   # 50% of range
                    tp2 = round(liq_pool, 8)                          # other side
                    sl_shift_level = round(sweep_high - range_size * 0.50, 8)  # SL→TP1 at 70%
                else:
                    # Range: sweep_low → liq_pool (the swing high before sweep)
                    sweep_low  = sweep_candle["low"]
                    range_size = abs(liq_pool - sweep_low)
                    tp1 = round(sweep_low  + range_size * 0.50, 8)
                    tp2 = round(liq_pool, 8)
                    sl_shift_level = round(sweep_low + range_size * 0.50, 8)

                if range_size <= 0:
                    continue

                risk   = abs(entry - sl_p)
                reward = abs(tp2 - entry)   # Use TP2 for RR calc
                if risk <= 0:
                    continue
                ...
                # ── Score ─────────────────────────────────────────────
                score = 0; details = []
                score += 20; details.append("✅ HTF Key Level tap (+20)")
                score += 15; details.append("✅ First-touch sweep confirmed (+15)")
                score += 20; details.append(f"✅ Displacement FVG on {ltf} (+20)")
                if in_pd:
                    score += 10; details.append(f"✅ {pd_name} zone (+10)")
                if (direction=="BUY" and trend=="BULLISH") or (direction=="SELL" and trend=="BEARISH"):
                    score += 10; details.append("✅ Trend aligned (+10)")
                if rr >= 4:   score += 10; details.append(f"✅ Strong {rr}R (+10)")
                elif rr >= 3: score += 7;  details.append(f"⚠️ {rr}R (+7)")
                else:         score += 4;  details.append(f"⚠️ {rr}R (+4)")
                score += 5; details.append(f"✅ TP1@50% / TP2@liquidity / SL trail@70% (+5)")
                score = min(score, 100)
                ...
                zone_label = kl.get("kl_type","KL")
                zone_name  = pd_name + " · " + zone_label
                ...
                results.append({
                    ...
                    "trend":        trend,
                    "entry":        entry,
                    "entry_type":   f"Model #2 (FVG Tip · {ltf})",
                    "sl":           sl_p,
                    "tp":           tp2,         # main TP stored as tp
                    "tp1":          tp1,         # 50% level
                    "tp2":          tp2,
                    "sl_shift_at":  sl_shift_level,   # trail SL here when 70% of range hit
                    "rr":           rr,
                    ...
                    "sweep_extreme": round(first_extreme, 8),
                    "tbs_found":    False,
                    "tbs_tf":       "–",
                    "tbs_entry":    "–",
                    "tbs_sl":       "–",
                ...
                break   # one signal per zone per LTF
            if results:
                break   # one signal per KL
        if results:
            break   # one signal per HTF
    return results
...
def place_order(sig):
    ...
    return True, f"Order placed: {sig['direction']} {sig['symbol']} size={size}"
    ...
    while True:
        try:
            ...
            for i,symbol in enumerate(pairs):
                ...
                try:
                    all_res = scan_pair(symbol) + scan_pair_model2(symbol)
                    for sig in all_res:
                        model_tag = sig.get("model","1")
                        recent_sigs = list(signals)[:50]
                        duplicate = any(
                            s.get("symbol")==sig["symbol"] and
                            s.get("direction")==sig["direction"] and
                            s.get("tf")==sig["tf"] and
                            s.get("model","1")==model_tag
                            for s in recent_sigs
                        )
                        if duplicate:
                            log(f"⏭ SKIP duplicate M#{model_tag}: {sig['direction']} {symbol} {sig['tf']}")
                            continue
                        diag["passed"]+=1
                        signals.appendleft(sig)
                        ...
                        if model_tag == "2":
                            log(f"🔥 M#2 {sig['direction']} {symbol} | HTF:{tf_lbl} | LTF:{sig['ob_tf']} | Score:{sig['score']} {sig['grade']} | RR:{sig['rr']}R | FVG entry")
...
<div class="tab-wrap">
  ...
  <div id="tab-signals">
    ...
    <div class="sig-list" id="slist"><div class="empty"><div class="empty-ico">🔭</div><div class="empty-t">Scanning the galaxy...</div><div class="empty-s">Hunting Mad Man Model #1 &amp; #2 setups. TBS mandatory (M#1) · FVG sweep entry (M#2). Min 2R. 🎯</div></div></div>
  </div>
  <!-- LIVE TRADES -->
  <div id="tab-trades" style="display:none">
    <div class="panel">
      <div id="live-bal-banner" style="display:flex;align-items:center;gap:12px;background:rgba(16,185,129,.08);border:1.5px solid rgba(16,185,129,.2);border-radius:14px;padding:10px 16px;margin-bottom:14px">
        <span style="font-size:1.2rem">💰</span>
        <div>
          <div style="font-family:'Fredoka One',sans-serif;font-size:.68rem;letter-spacing:.04em;color:rgba(167,139,250,.7)">MEXC FUTURES · AVAILABLE BALANCE</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:1rem;font-weight:700;color:var(--green)">$<span id="live-bal-val">–</span> <span style="font-size:.72rem;color:var(--dim)">USDT</span></div>
        </div>
        <div style="margin-left:auto;text-align:right">
          <div style="font-size:.62rem;color:var(--dim);font-family:'JetBrains Mono',monospace" id="live-bal-upd">–</div>
        </div>
      </div>
  ...
  <div id="tab-settings" style="display:none">
    ...
    <div class="panel">
      ...
      <div class="trade-form">
        ...
        <div class="tf-group">
          <div class="tf-lbl">MEXC Futures Available Balance</div>
          <div id="s-bal-banner" style="display:flex;align-items:center;gap:12px;background:rgba(16,185,129,.08);border:1.5px solid rgba(16,185,129,.22);border-radius:14px;padding:12px 16px;transition:all .3s">
            <span style="font-size:1.3rem">💰</span>
            <div>
              <div style="font-family:'Fredoka One',sans-serif;font-size:.72rem;letter-spacing:.04em;color:rgba(167,139,250,.7);margin-bottom:2px">PERPETUAL FUTURES · AVAILABLE</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:1.1rem;font-weight:700;color:var(--green)">$<span id="s-bal-val">–</span> <span style="font-size:.75rem;color:var(--dim)">USDT</span></div>
            </div>
            <button class="trade-btn tb-chk" style="padding:8px 14px;white-space:nowrap;margin-left:auto" onclick="loadSettingsBalance()">🔄 Refresh</button>
          </div>
          <div style="font-size:.68rem;color:var(--dim);margin-top:5px;font-family:'JetBrains Mono',monospace">Auto-refreshes every 30s · Only shows when API keys are configured</div>
        </div>
      </div>
    </div>
    <!-- Telegram -->
...
return`<div class="scard ${dir.toLowerCase()}"><div class="card-hdr"><span class="dtag ${dir}">${dir}</span><span class="csym">${s.symbol||"–"}</span><div class="chips"><span class="chip chip-tf">${isM2?"🔥 M#2":"🎯 M#1"} ${crtTF}</span>${!isND&&s.ob_tf&&s.ob_tf!=="N/A"?`<span class="chip chip-ob">${isM2?"FVG·"+obTF:zt+" "+obTF}</span>`:""}<span class="chip chip-tr ${s.trend}">${s.trend}</span>${isAplus?'<span class="chip chip-aplus">⭐ A+</span>':""} ${s.from_monitor?'<span class="chip" style="color:#fbbf24;border-color:rgba(251,191,36,.3);background:rgba(251,191,36,.07)">👁 Monitored</span>':""}</div><span class="gtag ${gc}">${gr}</span><span class="cts">${s.timestamp||""}</span></div>
${isM2?`<div class="lvl-grid"><div class="lv lv-e"><div class="lv-lbl">🎯 Entry (FVG Tip)</div><div class="lv-val">${fmt(s.entry)}</div></div><div class="lv lv-e" style="border-color:rgba(167,139,250,.2)"><div class="lv-lbl">Sweep Extreme</div><div class="lv-val" style="color:#f9a8d4">${fmt(s.sweep_extreme)}</div></div><div class="lv lv-s"><div class="lv-lbl">🛑 Stop Loss</div><div class="lv-val">${fmt(s.sl)}</div></div><div class="lv lv-t" style="border-color:rgba(251,191,36,.3)"><div class="lv-lbl">🎯 TP1 (50%)</div><div class="lv-val" style="color:#fbbf24">${fmt(s.tp1)}</div></div><div class="lv lv-t"><div class="lv-lbl">🏆 TP2 (Liquidity)</div><div class="lv-val">${fmt(s.tp2)}</div></div><div class="lv lv-r"><div class="lv-lbl">📊 RR</div><div class="lv-val">${s.rr}R</div></div><div class="lv" style="border-color:rgba(16,185,129,.25)"><div class="lv-lbl">🔔 Trail SL→TP1</div><div class="lv-val" style="color:var(--green);font-size:.72rem">When 70% hit</div></div></div>`:`<div class="lvl-grid"><div class="lv lv-e"><div class="lv-lbl">🎯 Entry (TBS Open)</div><div class="lv-val">${fmt(s.entry)}</div></div><div class="lv lv-e" style="border-color:rgba(167,139,250,.2)"><div class="lv-lbl">TBS TF</div><div class="lv-val" style="color:#a78bfa">${TFM[s.tbs_tf]||s.tbs_tf||"–"}</div></div><div class="lv lv-s"><div class="lv-lbl">🛑 Stop Loss</div><div class="lv-val">${fmt(s.sl)}</div></div><div class="lv lv-t"><div class="lv-lbl">🎯 Take Profit</div><div class="lv-val">${fmt(s.tp)}</div></div><div class="lv lv-r"><div class="lv-lbl">📊 RR</div><div class="lv-val">${s.rr}R</div></div><div class="lv"><div class="lv-lbl">CRH</div><div class="lv-val" style="color:#f9a8d4">${fmt(s.crh)}</div></div><div class="lv"><div class="lv-lbl">CRL</div><div class="lv-val" style="color:#6ee7b7">${fmt(s.crl)}</div></div></div>`}
${isM2?`<div class="cfms">${cf(true,"HTF Key Level")}${cf(true,"1st Touch Sweep")}${cf(true,"Displacement FVG")}${cfw(s.liq_swept,"Liquidity")}${cfg(isAplus,"A+")}<span class="cf cf-ok" style="color:#fbbf24">🔔 TP1@50% / TP2@Liq</span></div>`:`<div class="cfms">${cf(s.tbs_found,\`TBS \${TFM[s.tbs_tf]||s.tbs_tf||"?"}\`)}${cfw(s.fvg_found,s.fvg_type||"FVG")}${cfw(s.choch_found,"CHOCH")}${cfw(s.liq_swept,"Liq Sweep")}${cfw(s.ob_respected,"OB Resp")}${cfg(isAplus,"A+")}</div>`}
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades"){window.fetchPnl();fetchLiveBalance();}if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg")loadTradeConfig();if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings"){loadSettings();fetchLiveBalance();}};
...
async function loadSettings(){
  try{
    ...
    if(si("s-risk"))     si("s-risk").value      = d.risk_pct||1;
...
async function loadSettingsBalance(){
  ...
  const banner=document.getElementById("s-bal-banner");
  if(el) el.textContent="…";
  try{
    const r=await fetch("/api/balance");
    const d=await r.json();
    if(d.balance!=null && d.balance>0){
      const fmt=Number(d.balance).toFixed(2);
      if(el) el.textContent=fmt;
      if(banner){banner.style.borderColor="rgba(16,185,129,.4)";banner.style.background="rgba(16,185,129,.1)";}
      showSettingsMsg("✅ Balance: $"+fmt+" USDT",true);
    } else {
      if(el) el.textContent=d.error||"Check API keys";
      if(banner){banner.style.borderColor="rgba(239,68,68,.4)";banner.style.background="rgba(239,68,68,.07)";}
      showSettingsMsg("❌ "+(d.error||"Balance fetch failed — verify API keys"),false);
    }
  }catch(e){
    if(el)el.textContent="Error";
    showSettingsMsg("❌ Network error: "+e.message,false);
  }
}
function showSettingsMsg(msg,ok){const el=document.getElementById("settings-msg");if(!el)return;el.textContent=msg;el.className="t-status "+(ok===null?"ok":ok?"ok":"err");if(ok===null){el.style.background="rgba(245,158,11,.1)";el.style.borderColor="rgba(245,158,11,.3)";el.style.color="var(--yellow)";}else{el.style.background="";el.style.borderColor="";el.style.color="";}}
async function saveAllSettings(){
  const si=id=>document.getElementById(id);
  showSettingsMsg("💾 Saving…",null);
  const payload={
  ...
  try{
    ...
    if(d.ok){
      ...
      loadSettings();
    }else{showSettingsMsg("❌ Save failed — check console",false);}
  }catch(e){showSettingsMsg("❌ Error: "+e.message,false);}
  setTimeout(()=>{const el=document.getElementById("settings-msg");if(el){el.className="t-status";el.style.background="";el.style.borderColor="";el.style.color="";}},5000);
}
...
async function fetchLiveBalance(){
  try{
    const r=await fetch("/api/balance");
    const d=await r.json();
    // Update live trades banner
    const lv=document.getElementById("live-bal-val");
    const lu=document.getElementById("live-bal-upd");
    const lb=document.getElementById("live-bal-banner");
    // Update settings banner
    const sv=document.getElementById("s-bal-val");
    const sb=document.getElementById("s-bal-banner");
    if(d.balance!=null && d.balance>0){
      const fmt=Number(d.balance).toFixed(2);
      const ts="Updated "+new Date().toLocaleTimeString();
      if(lv)lv.textContent=fmt;
      if(lu)lu.textContent=ts;
      if(lb){lb.style.borderColor="rgba(16,185,129,.35)";lb.style.background="rgba(16,185,129,.08)";}
      if(sv)sv.textContent=fmt;
      if(sb){sb.style.borderColor="rgba(16,185,129,.4)";sb.style.background="rgba(16,185,129,.1)";}
    } else {
      const errMsg=d.error?"❌ "+d.error.substring(0,30):"❌ API keys needed";
      if(lv)lv.textContent=errMsg;
      if(lu)lu.textContent="";
      if(lb){lb.style.borderColor="rgba(239,68,68,.3)";lb.style.background="rgba(239,68,68,.06)";}
      if(sv)sv.textContent=d.error||"Check API keys";
      if(sb){sb.style.borderColor="rgba(239,68,68,.3)";sb.style.background="rgba(239,68,68,.06)";}
    }
  }catch(e){
    const lv=document.getElementById("live-bal-val");
...
def api_login():
    ...
    if data.get("password")==DASHBOARD_PASSWORD:
        ...
        resp.set_cookie("session",token,max_age=86400*7,httponly=True,samesite="Lax",secure=True)
        return resp
...
@app.route("/api/log")
def api_log():
    with log_lock: return jsonify({"log":list(scan_state["log"])})
...
# ── STARTUP ──────────────────────────────────────────────────────
# Railway uses Gunicorn — MUST use --workers 1 (see Procfile).
# _ensure_started is called both at module load AND on first request
# so the scanner always starts in whichever process handles traffic.
...
# Trigger on module load (works for python app.py and single-worker gunicorn)
try:
...
# Fallback: trigger on first HTTP request (catches any gunicorn edge cases)
@app.before_request
def _startup_hook():
    if not _scanner_started:
        _ensure_started()
