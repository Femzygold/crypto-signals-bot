...
SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "settings_persist.json")

def _load_persisted_settings():
    """Load settings saved to disk (survives redeployment if volume is mounted)."""
    try:
...
def _save_persisted_settings(data):
    """Save settings to disk so they survive restarts."""
    try:
...
# ── SETTINGS ──────────────────────────────────────────────────────────
# Priority: persisted file > environment variable > default
app_settings = {
    "telegram_bot_token": _persisted.get("telegram_bot_token") or os.environ.get("TELEGRAM_BOT_TOKEN", ""),
    "telegram_chat_id":   _persisted.get("telegram_chat_id")   or os.environ.get("TELEGRAM_CHAT_ID", "7411219487"),
    "signal_notify":      _persisted.get("signal_notify",  True),
    "trade_notify":       _persisted.get("trade_notify",   True),
    "price_interval":     _persisted.get("price_interval", 2),
    "scan_interval":      _persisted.get("scan_interval",  5),
    "cycle_rest":         _persisted.get("cycle_rest",     5),
}

# Keep global token in sync with whatever was loaded
TELEGRAM_BOT_TOKEN = app_settings["telegram_bot_token"] or TELEGRAM_BOT_TOKEN
TELEGRAM_CHAT_ID   = app_settings["telegram_chat_id"]   or TELEGRAM_CHAT_ID
...
def send_telegram(msg):
    tok = app_settings.get("telegram_bot_token","").strip()
    cid = app_settings.get("telegram_chat_id","").strip()
    if not tok or tok in ("PASTE_YOUR_TOKEN_HERE", ""):
        return False
    if not cid:
        return False
    try:
...
window.testTelegram = async function(){
  // Always save first so the token in the input is used in the test
  const token = ($("s-tg-token")||{}).value?.trim() || '';
  const chatId = ($("s-tg-chatid")||{}).value?.trim() || '';
  const savePayload = { signal_notify: _settingsNotify.signal, trade_notify: _settingsNotify.trade };
  if(token && token !== '***') savePayload.telegram_bot_token = token;
  if(chatId) savePayload.telegram_chat_id = chatId;
  if(!savePayload.telegram_bot_token && !token){
    showSettingsMsg("s-tg-msg","❌ Enter your bot token first, then click Send Test","false"); return;
  }
  try{ await fetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(savePayload)}); }catch{}
  showSettingsMsg("s-tg-msg","📤 Sending test message...",true);
  try{
    const r = await fetch("/api/telegram-test",{method:"POST"});
    const d = await r.json();
    showSettingsMsg("s-tg-msg", d.ok?"✅ Test message sent! Check Telegram.":"❌ "+d.error, d.ok);
  }catch{ showSettingsMsg("s-tg-msg","❌ Network error sending test",false); }
};
    ...
    if request.method == "POST":
        ...
        with settings_lock:
            ...
            _save_persisted_settings({
                k: v for k, v in app_settings.items()
            })
...
def api_telegram_test():
    ...
    try:
        ...
        cid = app_settings.get("telegram_chat_id","").strip()
        if not tok or tok in ("PASTE_YOUR_TOKEN_HERE", ""):
            return jsonify({"ok": False, "error": "No bot token saved. Enter your Telegram bot token and click Save first."})
        if not cid:
            return jsonify({"ok": False, "error": "No chat ID saved. Enter your Telegram chat ID and click Save first."})
        r = requests.post(
        ...
        err = d.get("description", "Unknown error")
        # Make common errors more human-readable
        if "bot was blocked" in err.lower():
            err = "Bot was blocked by the user. Open your Telegram bot and send /start first."
        elif "chat not found" in err.lower() or "not found" in err.lower():
            err = f"Chat not found. Make sure your Chat ID '{cid}' is correct and you have started a conversation with the bot."
        elif "unauthorized" in err.lower() or "invalid token" in err.lower():
            err = "Invalid bot token. Double-check your bot token from @BotFather."
        return jsonify({"ok": False, "error": err})
    except requests.exceptions.Timeout:
        return jsonify({"ok": False, "error": "Request timed out. Check your internet/proxy settings."})
    except Exception as e:
