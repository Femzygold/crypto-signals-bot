...
def is_continuous(sh, sl, direction, min_pts=2):
    """
    Accept structure if EITHER highs OR lows are printing correctly.
    Full alignment (both) = stronger signal (higher score).
    This stops perfectly valid setups being killed by one slightly off swing.
    """
    if direction=="BULLISH":
        ...
        return highs_ok or lows_ok   # at least one must be true
    else:
        if len(sh)<min_pts and len(sl)<min_pts: return False
        highs_ok = len(sh)>=min_pts and all(sh[i][1]<sh[i-1][1] for i in range(1,len(sh)))
        lows_ok  = len(sl)>=min_pts and all(sl[i][1]<sl[i-1][1] for i in range(1,len(sl)))
...
def ob_at_key_level(ob, direction, sh, sl, tol=0.025):
    """
    OB must be near the last Higher Low (bull) or Lower High (bear).
    Tolerance = 2.5%. Checks last 2 swing levels for flexibility.
    """
...
def fvg_in_pd_zone(candles, direction):
    """
    Find a Fair Value Gap on the HTF that is in the correct
    discount (bullish) or premium (bearish) zone.
    Returns (found, fvg_top, fvg_bot, zone_name)
    The CRT must form INSIDE this FVG zone.
    """
    ...
    for i in range(len(candles)-3, max(0, len(candles)-40), -1):
        ...
        if direction == "BULLISH":
            ...
            if c3["low"] > c1["high"]:
                ...
                # Must be in DISCOUNT zone (below 50%)
                if fvg_mid < eq:
            ...
            if c3["high"] < c1["low"]:
                ...
                fvg_bot = c3["high"]
                fvg_mid = (fvg_top + fvg_bot) / 2
                # Must be in PREMIUM zone (above 50%)
                if fvg_mid > eq:
...
def find_breaker_block(candles, direction):
    """
    Breaker Block (BB): a previously valid OB that was broken through.
    Bullish BB: old bearish OB that price broke above (now support).
    Bearish BB: old bullish OB that price broke below (now resistance).
    Returns list of BB dicts.
    """
    bbs = []
    ...
    for ob in obs:
        ...
        after = candles[idx+1:]
        if direction == "BULLISH":
            # Old bearish OB broken above = Bullish BB
...
def find_rejection_block(candles, direction):
    """
    Rejection Block (RJB): candle with a large wick rejected from a level,
...
def find_all_key_levels(candles, direction):
    """
    Find ALL valid key levels: OB, BB, FVG, IFVG, RJB.
    Returns list of zone dicts with kl_type field.
    """
    zones = []
    # Order Blocks
...
def crt_inside_zone(crt, zone_top, zone_bot):
    """Check if CRT candles overlap with the HTF OB or FVG zone."""
    crh = crt["crh"]; crl = crt["crl"]
    # CRT range must overlap the zone
...
def check_tbs(symbol, direction, crl, crh, crt_tf="Hour4"):
    """
    Turtle Body Soup — Model #1 entry.
    Uses the correct mapped LTF per CRT timeframe:
      1H CRT  -> 1m | 2H CRT -> 2-3m | 3H/4H CRT -> 3-4m | 1D CRT -> 45m-1H
    Bullish: body CLOSES below CRL, next candle recovers above CRL.
    Bearish: body CLOSES above CRH, next candle recovers below CRH.
    Entry = OPEN of TBS candle. SL = extreme of TBS candle.
    Returns: (found, tf, tbs_entry, tbs_sl)
    """
...
def score_signal(crt, trend, liq_swept, tbs_found, tbs_tf,
    ...
    # 1. Structure quality (20 pts)
    # Full alignment (both HH/HL or LH/LL) = 20pts, partial = 12pts
    if continuous:
    ...
    # A+ requires P/D zone confluence (passed via ob_zone containing "DISCOUNT" or "PREMIUM")
...
def fmt_tg(sig):
    ...
    return (
        f"{e} <b>CRT MODEL #1 — {sig['direction']}</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        ...
        f"<b>🎯 TP:</b>       {sig['tp']} ({'CRH' if sig['direction']=='SELL' else 'CRH'})\n"
        f"<b>📊 RR:</b>       {sig['rr']}R\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>CRH:</b>        {sig['crh']}\n"
        f"<b>CRL:</b>        {sig['crl']}\n"
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<i>CRT Scanner Model #1 • {sig['timestamp']}</i>"
    )
...
# Tracks pairs where C1 (CRT) is confirmed and C2 (manipulation) is forming
# Once C2 closes back inside → search for TBS → auto-trade
manip_monitor = {}   # symbol -> monitor dict
manip_lock    = threading.Lock()
MAX_MONITORED = 4

# ── RECENT TRADES STORE ──────────────────────────────────────────
recent_trades = deque(maxlen=10)   # last 10 completed trades

# Global diagnostic counters
diag = {
    "no_candles":0, "neutral":0, "not_continuous":0,
    "no_obs":0, "not_at_key":0, "not_in_zone":0,
    "not_tapping":0, "no_crts":0,
...
def scan_pair(symbol):
    ...
    if candles_1d and len(candles_1d)>=10:
        ...
        for crt in crts:
            ...
            # TBS on LTFs — Model #1: entry at TBS candle open
            tbs_found, tbs_tf, tbs_entry, tbs_sl = check_tbs(symbol, direction, crt["crl"], crt["crh"])
            if not tbs_found:
                diag["1d_no_tbs"]+=1; continue

            # Model #1 entry: open of TBS candle
            # SL: sweep extreme (TBS candle low/high)
            # TP: opposite CRT level
            if tbs_entry:
                entry      = tbs_entry
                entry_type = "Model #1 (TBS Open)"
                sl_p       = tbs_sl          # SL at the sweep
                tp_p       = crt["tp"]       # TP at CRH (for BUY) or CRL (for SELL)
            else:
                entry      = crt["entry"]
                entry_type = "C2 Close"
                sl_p       = crt["sl"]
            ...
            # CHOCH still useful for confluence scoring
            choch_found, choch_level = check_choch_multi(symbol, ["Min60","Min30"], direction)
            # FVG kept for scoring only, not entry
    ...
    # ── PATH B: CRT forming inside ANY valid key level (OB/BB/FVG/IFVG/RJB)
    # P/D zone is NOT mandatory — it upgrades the signal to A+
    for crt_tf in ["Hour4","Hour3","Hour2","Min60"]:
        ...
        for ob_tf in OB_TFS:
            ...
            # ── Find ALL key levels (OB, BB, FVG, IFVG, RJB) ─────────
            ...
            matched_ob  = None
            at_key      = False
            in_pd_zone  = False   # premium/discount — upgrades to A+

            for kl in all_kls[:8]:
                zt = kl["top"]; zb = kl["bot"]
                if zt <= zb: continue
                # Optional P/D zone check (not a hard gate)
                in_zone_pd, pd_name = ob_in_pd_zone(kl, ob_candles, direction)
                # Key level is valid regardless of P/D zone
                zone_found  = True
                zone_top    = zt
                zone_bot    = zb
            ...
            # ── Detect CRT forming INSIDE the zone ────────────────
            crts_raw = detect_crt(crt_candles, direction, ob=None)
            crts = [c for c in crts_raw if crt_inside_zone(c, zone_top, zone_bot)]

            # ── If no completed CRT, check for manip phase ────────
            if not crts:
                diag["no_crts"]+=1
                # Check if pair is in manipulation phase (C2 still forming)
                with manip_lock:
...
def get_symbol_info(symbol):
    """Get contract details for position sizing."""
    try:
...
def calc_position_size(symbol, entry, sl, balance):
    ...
    Position size (contracts) = risk_amount / (|entry - sl| * contract_size)
    """
...
def place_order(sig):
    """
    Place a futures order on MEXC based on CRT signal.
    Uses limit order at TBS candle open (Model #1 entry).
    ...
    # Max 80% of total balance across all 4 trades (4 x 20%)
    total_used = len(open_trades) * (balance * 0.20)
    if total_used >= balance * 0.80:
        return False, "30% balance cap reached across open trades"

    entry = float(sig["entry"])
    sl    = float(sig["sl"])
    tp    = float(sig["tp"])

    # Cross margin: 20% of total balance as margin per trade
    margin = balance * 0.20
    # Calc size from margin
    info = get_symbol_info(sig["symbol"])
    sl_dist = abs(entry - sl)
    if sl_dist <= 0: return False, "SL distance is zero"
    # SL loss must not exceed 100% of margin (cross margin rule)
    ...
    side = 1 if sig["direction"] == "BUY" else 2  # 1=Long, 2=Short
    open_type = 2  # Cross margin

    # ── Auto-calculate leverage to keep SL within 70% of margin ──
    is_aplus = sig.get("grade") == "A+"
    # Cross margin: 20% of total futures balance as margin per trade
    margin = balance * 0.20
    # SL distance as % of entry
    sl_pct = abs(entry - sl) / entry if entry > 0 else 0.01
    # Max leverage where SL loss = 70% of margin
    # SL loss must not exceed 100% of margin
    # loss = margin * sl_pct * leverage => leverage = 1.0 / sl_pct
    if sl_pct > 0:
        max_safe_lev = int(1.0 / sl_pct)   # 100% margin cap
    else:
        max_safe_lev = 10
    # Up to 500x on all setups, cross margin
    min_lev = 10
    max_lev = 500   # always allow up to 500x
    leverage = max(min_lev, min(max_lev, max_safe_lev))
    # Override with user config if lower
...
def close_trade(symbol, reason="Manual"):
    ...
    side = 2 if trade["direction"] == "BUY" else 1  # opposite side to close
    params = {
        "symbol":    symbol,
...
def detect_manip_phase(candles, direction):
    """
    Detect pairs where C1 (CRT candle) is confirmed but C2 (manipulation)
    has NOT yet closed back inside. We monitor these.
    Returns list of pending manip dicts.
    """
    pending = []
    if len(candles) < 4: return pending
    limit = min(10, len(candles)-1)
    for offset in range(1, limit):
        i2 = len(candles)-1-offset   # C2 = current/last candle (still forming or just closed)
        i1 = i2-1
        if i1 < 0: break
        ...
        if cr_range <= 0: continue
        if direction == "BULLISH":
            # C2 has swept below CRL but NOT yet closed back inside
            swept = c2["low"] < crl
            still_below = c2["close"] < crl   # still outside = still in manip
...
def check_manip_completed(symbol, monitor):
    """
    Check if a monitored manipulation phase has completed
    (C2 closed back inside CRT range).
    Returns (completed, new_candles)
    """
    ...
    # Look at last 5 candles for completion
    recent = candles[-5:]
    for c in recent:
        if direction == "BUY":
            # Completed: candle body closed back above CRL
...
def manip_monitor_loop():
    ...
    1. Watches pairs in manip_monitor dict
    2. When C2 completes (closes back inside range) → search TBS → auto-trade
    3. Invalidates if sweep candle closes WITH WICK beyond CRH/CRL
    """
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if candles:
                    ...
                    crh = monitor["crh"]; crl = monitor["crl"]
                    direction = monitor["direction"]
                    # Invalidation: sweep candle has a WICK beyond the level
                    # i.e. C2 closed with body beyond CRH/CRL (not just wick)
                    # Per strategy: if FIRST sweep candle closes with body beyond = invalid
                    c2 = monitor.get("c2",{})
                    if direction == "BUY":
                        # Invalid if C2 body fully below CRL (body, not just wick)
                ...
                # Get zone/trend info from monitor
                zone_name = monitor.get("zone_name","–")
                trend     = monitor.get("trend","NEUTRAL")
                kl_type   = monitor.get("kl_type","KL")
                crt_tf_label = {"Day1":"1D","Hour4":"4H","Hour3":"3H","Hour2":"2H","Min60":"1H"}.get(crt_tf,"–")
...
def scanner_loop():
    ...
    while True:
        try:
            ...
            log(f"📊 GATE STATS: neutral={diag['neutral']} no_cont={diag['not_continuous']} "
                f"no_obs={diag['no_obs']} not_key={diag['not_at_key']} not_zone={diag['not_in_zone']} "
                f"no_liq={diag['no_liq']} not_tap={diag['not_tapping']} no_crt={diag['no_crts']} "
                f"no_tbs={diag['no_tbs']} rr_low={diag['rr_low']}")
            log(f"📊 1D STATS: no_crt={diag['1d_no_crts']} no_tbs={diag['1d_no_tbs']} rr_low={diag['1d_rr_low']} PASSED={diag['passed']}")
            # Reset counters
...
function login(){
  ...
  fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pw})})
    .then(r=>r.json()).then(d=>{
      if(d.ok){btn.textContent='✅ Let\'s go!';setTimeout(()=>window.location.href='/dashboard',300);}
    ...
    }).catch(()=>{btn.textContent='🔓 Enter Dashboard';btn.disabled=false;});
}
...
(function(){
'use strict';
var allSigs=[],toastT,activeTab='signals',tick=0,lastCount=0;
...
function toast(m,t,d){
  ...
  var el=$('toast');
  el.textContent=m;
  el.className='toast show'+(t==='buy'?' bt':t==='sell'?' st':t==='trade'?' tt':'');
  clearTimeout(toastT);
  toastT=setTimeout(function(){el.classList.remove('show');},d);
}

function scoreColor(s){
  return s>=88?'#fbbf24':s>=75?'#a78bfa':s>=60?'var(--blue)':s>=45?'var(--orange)':'var(--dim)';
}
...
var TFM={Day1:'1D',Hour4:'4H',Hour3:'3H',Hour2:'2H',Min60:'1H',Min45:'45m',Min30:'30m',Min15:'15m',Min10:'10m',Min5:'5m',Min4:'4m',Min3:'3m',Min2:'2m',Min1:'1m'};
var TOP=['BTC_USDT','ETH_USDT','SOL_USDT','BNB_USDT','XRP_USDT','DOGE_USDT'];
...
async function fetchPrices(){
  try{
    ...
    for(var i=0;i<TOP.length;i++){
      var sym=TOP[i];
      var name=sym.replace('_USDT','');
      var d=data[sym];
      if(!d){html+='<div class="pc"><div class="pc-sym">'+name+'/USDT</div><div class="pc-price" style="color:var(--dim)">-</div></div>';continue;}
      var up=d.change>=0;
      var cls=up?'up':'dn';
      html+='<div class="pc '+cls+'">'
           +'<div class="pc-sym">'+name+'/USDT</div>'
           +'<div class="pc-price '+cls+'">'+fmtP(d.price)+'</div>'
           +'<span class="pc-chg '+cls+'">'+(up?'&#9650;':'&#9660;')+' '+Math.abs(d.change||0).toFixed(2)+'%</span>'
           +'</div>';
    }
    $('pgrid').innerHTML=html;
  }catch(e){console.log('prices err',e);}
}

// == SIGNAL CARDS ==
function buildCard(s,idx){
  var dir=(s.direction||'BUY').toUpperCase();
  var sc=s.score||0, gr=s.grade||'-';
  var gc={'+':'gAp','A':'gA','B':'gB','C':'gC','D':'gD'}[gr.replace('A','')]||'gD';
  if(gr==='A+')gc='gAp'; else if(gr==='A')gc='gA'; else if(gr==='B')gc='gB'; else if(gr==='C')gc='gC'; else gc='gD';
  var crtTF=TFM[s.tf]||s.tf||'-';
  var obTF=TFM[s.ob_tf]||s.ob_tf||'-';
  var isND=s.tf==='Day1';
  var zt=s.zone_type||s.ob_zone||'-';
  var isAplus=gr==='A+';
  var sc_color=scoreColor(sc);

  // chips
  var chips='<span class="chip chip-tf">'+crtTF+' CRT</span>';
  if(!isND&&s.ob_tf&&s.ob_tf!=='N/A')chips+='<span class="chip chip-ob">'+zt+' '+obTF+'</span>';
  chips+='<span class="chip chip-tr '+s.trend+'">'+s.trend+'</span>';
  if(isAplus)chips+='<span class="chip chip-aplus">&#11088; A+</span>';
  if(s.from_monitor)chips+='<span class="chip" style="color:#fbbf24;border-color:rgba(251,191,36,.3);background:rgba(251,191,36,.07)">&#128065; Monitored</span>';

  // levels
  var lvls='';
  lvls+='<div class="lv lv-e"><div class="lv-lbl">&#127919; Entry (TBS Open)</div><div class="lv-val">'+fmt(s.entry)+'</div></div>';
  lvls+='<div class="lv lv-e" style="border-color:rgba(167,139,250,.2)"><div class="lv-lbl">TBS TF</div><div class="lv-val" style="color:#a78bfa">'+(TFM[s.tbs_tf]||s.tbs_tf||'-')+'</div></div>';
  lvls+='<div class="lv lv-s"><div class="lv-lbl">&#128721; Stop Loss</div><div class="lv-val">'+fmt(s.sl)+'</div></div>';
  lvls+='<div class="lv lv-t"><div class="lv-lbl">&#127919; Take Profit</div><div class="lv-val">'+fmt(s.tp)+'</div></div>';
  lvls+='<div class="lv lv-r"><div class="lv-lbl">&#128202; RR</div><div class="lv-val">'+s.rr+'R</div></div>';
  lvls+='<div class="lv"><div class="lv-lbl">CRH</div><div class="lv-val" style="color:#f9a8d4">'+fmt(s.crh)+'</div></div>';
  lvls+='<div class="lv"><div class="lv-lbl">CRL</div><div class="lv-val" style="color:#f9a8d4">'+fmt(s.crl)+'</div></div>';
  if(s.ob_top&&s.ob_top!=='-')lvls+='<div class="lv lv-o"><div class="lv-lbl">'+zt+' Top</div><div class="lv-val">'+fmt(s.ob_top)+'</div></div>';
  if(s.ob_bot&&s.ob_bot!=='-')lvls+='<div class="lv lv-o"><div class="lv-lbl">'+zt+' Bot</div><div class="lv-val">'+fmt(s.ob_bot)+'</div></div>';

  // confirms
  function cf(ok,l){return '<span class="cf '+(ok?'cf-ok':'cf-no')+'">'+(ok?'&#10003;':'&#10007;')+' '+l+'</span>';}
  function cfw(ok,l){return '<span class="cf '+(ok?'cf-ok':'cf-w')+'">'+(ok?'&#10003;':'&#9888;')+' '+l+'</span>';}
  function cfg(ok,l){return '<span class="cf '+(ok?'cf-g':'cf-no')+'">'+(ok?'&#9830;':'&#9826;')+' '+l+'</span>';}
  var cfms=cf(s.tbs_found,'TBS '+(TFM[s.tbs_tf]||s.tbs_tf||'-'))
          +cfw(s.choch_found,'CHOCH')
          +cfg(s.ob_zone&&s.ob_zone!=='-',s.ob_zone||'Zone')
          +cfw(s.continuous,'Structure');

  var det='';
  if(s.details&&s.details.length){
    var dtxt=(s.details||[]).join('\n');
    det='<button class="dettog" onclick="toggleDet('+idx+')">&#9654; Score Breakdown</button>'
       +'<div class="detbox" id="det-'+idx+'"><pre style="font-size:.62rem;line-height:1.85;white-space:pre-wrap">'+dtxt+'</pre></div>';
  }
  ...
  return '<div class="scard '+dir.toLowerCase()+'">'
    ...
    +'</div>'
    +'<div class="lvl-grid">'+lvls+'</div>'
    +'<div class="cfms">'+cfms+'</div>'
    +'<div class="srow">'
      +'<span class="slbl">Score</span>'
      +'<div class="strack"><div class="sfill" style="width:'+sc+'%;background:'+sc_color+'"></div></div>'
      +'<span class="snum" style="color:'+sc_color+'">'+sc+'/100</span>'
    +'</div>'
    +det
  +'</div>';
}
...
window.toggleDet=function(i){
  ...
  if(t)t.textContent=b.classList.contains('open')?'&#9660; Score Breakdown':'&#9654; Score Breakdown';
};

window.renderSigs=function(){
  var dF=$('fd').value, gF=$('fg').value, tfF=$('ftf').value;
  var f=allSigs.filter(function(s){
    if(dF&&s.direction!==dF)return false;
    if(tfF&&s.tf!==tfF)return false;
    if(gF==='A+'&&s.grade!=='A+')return false;
    if(gF==='A'&&s.grade!=='A')return false;
    if(gF==='B'&&(s.grade==='C'||s.grade==='D'))return false;
    return true;
  });
  var list=$('slist');
  if(!f.length){
    list.innerHTML='<div class="empty"><div class="empty-ico">&#128301;</div><div class="empty-t">Scanning the galaxy...</div><div class="empty-s">Hunting CRT Model #1 setups. TBS body close mandatory. Min 2R.</div></div>';
    return;
  }
  var html='';
  var shown=f.slice(0,100);
  for(var i=0;i<shown.length;i++)html+=buildCard(shown[i],i);
  list.innerHTML=html;
};

async function fetchSigs(){
  try{
    var r=await fetch('/api/signals?limit=200');
    var data=await r.json();
    allSigs=data;
    if(data.length>lastCount&&lastCount>0){
      var n=data[0];
      toast('Signal: '+n.direction+' '+n.symbol+' '+n.score+'/100 '+n.grade+' '+n.rr+'R',n.direction==='BUY'?'buy':'sell');
    }
...
async function fetchStats(){
  try{
    ...
    var d=await r.json();
    $('st').textContent=d.total||0;
    $('sb').textContent=d.buys||0;
    $('ss').textContent=d.sells||0;
  }catch(e){}
}

async function fetchState(){
  try{
    var r=await fetch('/api/scan-state');
    var d=await r.json();
    var pct=d.total_pairs>0?Math.round(d.pairs_done/d.total_pairs*100):0;
    $('pfill').style.width=pct+'%';
    $('pcnt').textContent=d.pairs_done+'/'+d.total_pairs;
    $('cpair').textContent=d.current_pair?'Scanning: '+d.current_pair:'Waiting...';
    $('sc2').textContent=d.scan_count||0;
    $('sl2').textContent=d.last_scan?'Last: '+d.last_scan:'-';
    $('snum').textContent='Scan #'+(d.scan_count||0);
    var en=d.enabled!==false;
    $('tbtn').textContent=en?'Stop':'Resume';
    $('tbtn').className='tbtn '+(en?'on':'off');
    $('sdot').className='sdot'+(en?'':' off');
    $('stxt').textContent=en?'SCANNING...':'PAUSED';
    $('stxt').className='stxt'+(en?'':' off');
...
async function fetchMonitor(){
  ...
  try{
    ...
    var data=await r.json();
    $('smon').textContent=data.length;
    $('mon-count').textContent='('+data.length+'/4)';
    var wrap=$('monitor-wrap');
    if(!wrap)return;
    if(!data.length){
      wrap.innerHTML='<div class="empty" style="padding:40px"><div class="empty-ico">&#127769;</div><div class="empty-t">Nothing monitored</div><div class="empty-s">Pairs in manipulation phase appear here automatically</div></div>';
      return;
    }
    var html='<div class="monitor-grid">';
    for(var i=0;i<data.length;i++){
      var m=data[i];
      var dir=(m.direction||'BUY').toUpperCase();
      var tf=TFM[m.crt_tf]||m.crt_tf||'-';
      html+='<div class="mon-card '+dir.toLowerCase()+'">'
           +'<div class="mon-sym">'+(dir==='BUY'?'&#128994;':'&#128308;')+' '+(m.symbol||'-')+'</div>'
           +'<div class="mon-row"><span>CRT TF</span><span>'+tf+'</span></div>'
           ...
           +'<div class="mon-row"><span>Zone</span><span>'+(m.zone_name||'-')+'</span></div>'
           +'<div class="mon-row"><span>Added</span><span>'+(m.added_at||'-')+'</span></div>'
           +'<div class="mon-status">&#9203; AWAITING C2 CLOSE</div>'
           +'</div>';
    }
    html+='</div>';
    wrap.innerHTML=html;
...
window.fetchPnl=async function(){
  ...
  try{
    var tr=await fetch('/api/trades').then(function(r){return r.json();});
    var pnlData=await fetch('/api/pnl').then(function(r){return r.json();});
    $('trades-count').textContent='('+tr.length+')';
    var wrap=$('live-trades-wrap');
    if(!wrap)return;
    if(!tr.length){
    ...
    var pnlMap={};
    var pos=pnlData.positions||[];
    for(var i=0;i<pos.length;i++)pnlMap[pos[i].symbol]=pos[i];
    var html='<div style="overflow-x:auto"><table class="tbl"><thead><tr>'
      +'<th>Pair</th><th>Dir</th><th>Entry</th><th>Current</th><th>SL</th><th>TP</th>'
      +'<th>RR</th><th>Lev</th><th>Margin</th><th>PnL</th><th>ROI%</th><th>Grade</th><th>Action</th>'
      +'</tr></thead><tbody>';
    for(var i=0;i<tr.length;i++){
      var t=tr[i];
      var live=pnlMap[t.symbol]||{};
      var pv=live.pnl||0; var roi=live.roi_pct||0;
      var cur=live.current||0; var lev=live.leverage||t.leverage||'-';
      var margin=((live.margin||0)).toFixed(2);
      var pvCls=pv>=0?'pos':'neg'; var roiCls=roi>=0?'pos':'neg';
      html+='<tr>'
        +'<td style="font-weight:800;color:var(--text)">'+t.symbol+'</td>'
        +'<td class="'+(t.direction==='BUY'?'buy':'sell')+'">'+t.direction+'</td>'
        +'<td>'+fmt(t.entry)+'</td>'
        +'<td style="color:var(--yellow)">'+(cur?fmt(cur):'-')+'</td>'
        +'<td style="color:var(--red)">'+fmt(t.sl)+'</td>'
        +'<td style="color:var(--green)">'+fmt(t.tp)+'</td>'
        +'<td style="color:var(--yellow)">'+t.rr+'R</td>'
        +'<td style="color:#a78bfa">'+lev+'x</td>'
        +'<td>$'+margin+'</td>'
        +'<td class="pos-pnl '+pvCls+'">'+(pv>=0?'+':'')+pv.toFixed(2)+'</td>'
        +'<td class="pos-pnl '+roiCls+'">'+(roi>=0?'+':'')+roi.toFixed(2)+'%</td>'
        +'<td style="color:'+scoreColor(t.score||0)+';font-family:Fredoka One,sans-serif">'+(t.grade||'-')+'</td>'
        +'<td><button class="action-btn close-btn" onclick="closeTrade(\''+t.symbol+'\')">x Close</button></td>'
        +'</tr>';
    }
    html+='</tbody></table></div>';
    wrap.innerHTML=html;
  }catch(e){console.log('pnl err',e);}
};
...
async function fetchHistory(){
  ...
  try{
    ...
    var html='<div style="overflow-x:auto"><table class="tbl"><thead><tr>'
      +'<th>Pair</th><th>Dir</th><th>Entry</th><th>SL</th><th>TP</th>'
      +'<th>RR</th><th>Grade</th><th>Status</th><th>Opened</th><th>Share</th>'
      +'</tr></thead><tbody>';
    for(var i=0;i<data.length;i++){
      var t=data[i];
      html+='<tr>'
        ...
        +'<td style="color:var(--yellow)">'+t.rr+'R</td>'
        +'<td style="color:'+scoreColor(t.score||0)+';font-family:Fredoka One,sans-serif">'+(t.grade||'-')+'</td>'
        +'<td style="color:var(--dim);font-size:.62rem">'+(t.status||'-')+'</td>'
        +'<td style="color:var(--dim)">'+(t.opened_at||'').replace(' UTC','')+'</td>'
        +'<td><button class="action-btn share-btn" onclick="showShare('+i+')">&#128247; Share</button></td>'
        +'</tr>';
    }
    html+='</tbody></table></div>';
    wrap.innerHTML=html;
...
window.showShare=function(i){
  ...
  var html='<div style="font-size:1.3rem;margin-bottom:10px">'+(t.direction==='BUY'?'&#128994;':'&#128308;')+' '+dir+' '+t.symbol+'</div>';
  html+='<div class="sh-row"><span class="sh-lbl">Entry</span><span class="sh-val">'+fmt(t.entry)+'</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Stop Loss</span><span class="sh-val" style="color:var(--red)">'+fmt(t.sl)+'</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Take Profit</span><span class="sh-val" style="color:var(--green)">'+fmt(t.tp)+'</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Risk:Reward</span><span class="sh-val" style="color:var(--yellow)">'+t.rr+'R</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Grade</span><span class="sh-val">'+(t.grade||'-')+'</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Strategy</span><span class="sh-val">CRT Model #1</span></div>';
  html+='<div class="sh-row"><span class="sh-lbl">Status</span><span class="sh-val">'+(t.status||'-')+'</span></div>';
...
async function fetchLog(){
  ...
  try{
    ...
    for(var i=0;i<d.log.length;i++){
      var l=d.log[i];
      var cls=l.indexOf('Signal')>=0||l.indexOf('SIGNAL')>=0?'ll-s':
              l.indexOf('TRADE')>=0||l.indexOf('AUTO')>=0?'ll-t':
              l.indexOf('Error')>=0||l.indexOf('error')>=0?'ll-e':
              l.indexOf('MONITOR')>=0||l.indexOf('MANIP')>=0?'ll-m':'ll-i';
      html+='<div class="'+cls+'">'+l+'</div>';
    }
    $('lbody').innerHTML=html;
  }catch(e){}
  try{
    var r2=await fetch('/api/diag');
    var d2=await r2.json();
    var labels={neutral:'Neutral',not_continuous:'Structure',no_obs:'No OBs',not_at_key:'Not Key',
                not_in_zone:'Zone',not_tapping:'Tapping',no_crts:'No CRT',no_tbs:'No TBS',
                rr_low:'Low RR','1d_no_crts':'1D NoCRT','1d_no_tbs':'1D NoTBS','1d_rr_low':'1D LowRR',passed:'PASSED'};
    var colors={neutral:'var(--dim)',not_continuous:'var(--dim)',no_obs:'var(--orange)',
                not_at_key:'var(--orange)',not_in_zone:'var(--orange)',not_tapping:'var(--red)',
                no_crts:'var(--red)',no_tbs:'var(--red)',rr_low:'var(--orange)',
                '1d_no_crts':'var(--dim)','1d_no_tbs':'var(--dim)','1d_rr_low':'var(--dim)',passed:'var(--green)'};
    var grid=$('diag-grid');
    if(!grid)return;
    var dhtml='';
    for(var k in d2){
      dhtml+='<div class="dg"><div class="dg-lbl">'+(labels[k]||k)+'</div>'
            +'<div class="dg-val" style="color:'+(colors[k]||'var(--text)')+'">'+d2[k]+'</div></div>';
    }
...
async function loadTradeConfig(){
  try{
    var r=await fetch('/api/trade-config');
    var d=await r.json();
    if(d.api_key)$('t-apikey').value=d.api_key;
    $('t-risk').value=d.risk_pct||1;
    updateTradeBadge(d.enabled);
  }catch(e){}
}

function updateTradeBadge(en){
  var b=$('trade-badge');
  var eb=$('t-enable-btn');
  var db=$('t-disable-btn');
  if(en){
    b.textContent='ENABLED';
    b.style.cssText='font-size:.7rem;padding:3px 10px;border-radius:8px;background:rgba(16,185,129,.12);border:1.5px solid rgba(16,185,129,.35);color:var(--green);font-family:JetBrains Mono,monospace;font-weight:700';
    if(eb)eb.style.display='none';
    if(db)db.style.display='';
  }else{
    b.textContent='DISABLED';
    b.style.cssText='font-size:.7rem;padding:3px 10px;border-radius:8px;background:rgba(239,68,68,.1);border:1.5px solid rgba(239,68,68,.3);color:var(--red);font-family:JetBrains Mono,monospace;font-weight:700';
    if(eb)eb.style.display='';
    if(db)db.style.display='none';
  }
}

function showTradeMsg(msg,ok){
  var el=$('trade-msg');
  el.textContent=msg;
  el.className='t-status '+(ok?'ok':'err');
  setTimeout(function(){el.className='t-status';},4000);
}

window.saveTradeConfig=async function(){
  var cfg={
    api_key:$('t-apikey').value.trim(),
    api_secret:$('t-secret').value.trim(),
    risk_pct:parseFloat($('t-risk').value)||1
  };
  if(!cfg.api_key){showTradeMsg('API key required',false);return;}
  try{
    var r=await fetch('/api/trade-config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)});
    var d=await r.json();
    if(d.ok){showTradeMsg('Settings saved!',true);toast('Settings saved!','trade');}
    else showTradeMsg('Save failed',false);
  }catch(e){showTradeMsg('Network error',false);}
};

window.enableTrade=async function(en){
  try{
    var r=await fetch('/api/trade-config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:en})});
    var d=await r.json();
    if(d.ok){
      updateTradeBadge(en);
      showTradeMsg(en?'Auto-trade ENABLED!':'Auto-trade disabled',true);
      toast(en?'Auto-trade ON!':'Auto-trade OFF','trade');
    }
  }catch(e){showTradeMsg('Error',false);}
};
...
window.fetchBalance=async function(){
  try{
    ...
    var d=await r.json();
    if(d.error){showTradeMsg('Balance error: '+d.error,false);}
    else{
      $('bal-val').textContent=Number(d.balance).toFixed(2);
      showTradeMsg('Balance loaded',true);
    }
  }catch(e){showTradeMsg('Check API keys',false);}
};

window.closeTrade=async function(sym){
  if(!confirm('Close '+sym+' position?'))return;
  try{
    var r=await fetch('/api/trade-close',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:sym})});
    var d=await r.json();
    toast(d.ok?sym+' closed':'Close failed: '+d.message,'trade');
    await window.fetchPnl();
  }catch(e){toast('Close failed','trade');}
};

// == SCANNER TOGGLE ==
window.toggleScanner=async function(){
  try{
    var r=await fetch('/api/toggle-scanner',{method:'POST'});
    var d=await r.json();
    toast(d.enabled?'Scanner resumed!':'Scanner stopped!',d.enabled?'buy':'sell');
    await fetchState();
  }catch(e){toast('Toggle error','');}
};
...
window.sw=function(tab,btn){
  ...
  var tabIds=['signals','trades','monitor','history','trade-cfg','log'];
  for(var i=0;i<tabIds.length;i++){
    var el=document.getElementById('tab-'+tabIds[i]);
    if(el)el.style.display=(tabIds[i]===tab)?'block':'none';
  }
  if(tab==='log')fetchLog();
  if(tab==='trades')window.fetchPnl();
  if(tab==='monitor')fetchMonitor();
  if(tab==='history')fetchHistory();
  if(tab==='trade-cfg')loadTradeConfig();
};

window.logout=function(){
  fetch('/api/logout',{method:'POST'}).finally(function(){window.location.href='/';});
};

// == POLL LOOP ==
async function poll(){
  tick++;
  var ps=[fetchSigs(),fetchStats(),fetchState()];
  if(tick%2===0)ps.push(fetchPrices());
  if(activeTab==='log'&&tick%2===0)ps.push(fetchLog());
  if(activeTab==='monitor'&&tick%2===0)ps.push(fetchMonitor());
  if(activeTab==='trades'&&tick%3===0)ps.push(window.fetchPnl());
  if(activeTab==='history'&&tick%5===0)ps.push(fetchHistory());
...
.lv-lbl{font-family:'JetBrains Mono',monospace;font-size:.53rem;color:var(--dim);letter-spacing:.07em;text-transform:uppercase;margin-bottom:5px;font-weight:700}
...
.snum{font-family:'Fredoka One',sans-serif;font-size:.95rem;white-space:nowrap;width:60px;text-align:right}
...
.ll-s{color:var(--green)}.ll-e{color:var(--red)}.ll-i{color:rgba(56,189,248,.7)}.ll-t{color:#f9a8d4}.ll-m{color:var(--yellow)}
...
.toast.bt{border-color:rgba(16,185,129,.4);color:var(--green)}.toast.st{border-color:rgba(239,68,68,.4);color:var(--red)}.toast.tt{border-color:rgba(249,115,22,.4);color:var(--orange)}
...
<header class="hdr">
  ...
  <div class="hdr-in">
    <div class="brand"><span class="brand-icon">📡</span><div><div class="brand-name">CRT Scanner</div><div class="brand-sub">MEXC USDT PERP · CROSS MARGIN · UP TO 500X</div></div></div>
...
<div class="tab-wrap">
  ...
  <div id="tab-signals">
    <div class="frow">
      <div class="ftitle">🎯 CRT Model #1</div>
      <div class="fgrp">
    ...
    <div class="sig-list" id="slist"><div class="empty"><div class="empty-ico">🔭</div><div class="empty-t">Scanning the galaxy...</div><div class="empty-s">Hunting CRT Model #1 setups. TBS body close mandatory. Min 2R. 🎯</div></div></div>
  </div>
  <!-- LIVE TRADES -->
  ...
  <div id="tab-trade-cfg" style="display:none">
    <div class="panel">
      ...
      <div class="trade-actions">
        ...
        <button class="trade-btn tb-on" id="t-enable-btn" onclick="enableTrade(true)">▶ Enable Auto-Trade</button>
        <button class="trade-btn tb-off" id="t-disable-btn" onclick="enableTrade(false)" style="display:none">⏹ Disable Auto-Trade</button>
  ...
  <div id="tab-log" style="display:none">
    ...
    <div class="log-wrap">
      <div class="log-hdr"><span class="log-ttl">🖥️ Live Log</span><span class="log-sub">UPDATES EVERY 3S</span></div>
...
(function(){
'use strict';
let allSigs=[],toastT,activeTab='signals',tick=0,lastCount=0;
const $=id=>document.getElementById(id);
function toast(m,t,d=3500){const el=$("toast");el.textContent=m;el.className="toast show"+(t==="buy"?" bt":t==="sell"?" st":t==="trade"?" tt":"");clearTimeout(toastT);toastT=setTimeout(()=>el.classList.remove("show"),d);}
...
async function fetchPrices(){
  try{
    ...
    for(var i=0;i<TOP.length;i++){
      ...
      var d=data[sym];
      if(!d){
        html+='<div class="pc"><div class="pc-sym">'+name+'/USDT</div><div class="pc-price" style="color:var(--dim)">–</div></div>';
        continue;
      }
      var up=d.change>=0;
      var cls=up?"up":"dn";
      var arr=up?"▲":"▼";
      var chg=Math.abs(d.change||0).toFixed(2);
      var price=fmtP(d.price);
      html+='<div class="pc '+cls+'"><div class="pc-sym">'+name+'/USDT</div><div class="pc-price '+cls+'">'+price+'</div><span class="pc-chg '+cls+'">'+arr+' '+chg+'%</span></div>';
    }
    $("pgrid").innerHTML=html;
  }catch(e){console.log("prices err",e);}
}
function buildCard(s,idx){
  var dir=(s.direction||"BUY").toUpperCase();
  var sc=s.score||0,gr=s.grade||"–";
  var gc={"A+":"gAp","A":"gA","B":"gB","C":"gC","D":"gD"}[gr]||"gD";
  var crtTF=TFM[s.tf]||s.tf||"–";
  var obTF=TFM[s.ob_tf]||s.ob_tf||"–";
  var isND=s.tf==="Day1";
  var zt=s.zone_type||s.ob_zone||"–";
  var isAplus=gr==="A+";
  var details=(s.details||[]).join("\n");
  function cf(ok,l){return'<span class="cf '+( ok?"cf-ok":"cf-no")+'">'+( ok?"✓":"✗")+' '+l+'</span>';}
  function cfw(ok,l){return'<span class="cf '+( ok?"cf-ok":"cf-w")+'">'+( ok?"✓":"⚠")+' '+l+'</span>';}
  function cfg(ok,l){return'<span class="cf '+( ok?"cf-g":"cf-no")+'">'+( ok?"💎":"◇")+' '+l+'</span>';}
  var chips="";
  chips+='<span class="chip chip-tf">'+crtTF+' CRT</span>';
  if(!isND&&s.ob_tf&&s.ob_tf!=="N/A") chips+='<span class="chip chip-ob">'+zt+' '+obTF+'</span>';
  chips+='<span class="chip chip-tr '+s.trend+'">'+s.trend+'</span>';
  if(isAplus) chips+='<span class="chip chip-aplus">⭐ A+</span>';
  if(s.from_monitor) chips+='<span class="chip" style="color:#fbbf24;border-color:rgba(251,191,36,.3);background:rgba(251,191,36,.07)">👁 Monitored</span>';
  ...
  if(s.ob_bot&&s.ob_bot!=="–") lvls+='<div class="lv lv-o"><div class="lv-lbl">'+zt+' Bot</div><div class="lv-val">'+fmt(s.ob_bot)+'</div></div>';
  var cfms="";
  cfms+=cf(s.tbs_found,"TBS · "+(TFM[s.tbs_tf]||s.tbs_tf||"–"));
  cfms+=cfw(s.choch_found,"CHOCH");
  cfms+=cfg(s.ob_zone&&s.ob_zone!=="–",s.ob_zone||"Zone");
  cfms+=cfw(s.continuous,"Structure");
  var sc_color=scoreColor(sc);
  var det_html="";
  if(details) det_html='<button class="dettog" onclick="toggleDet('+idx+')">▶ Score Breakdown</button><div class="detbox" id="det-'+idx+'"><pre style="font-size:.62rem;line-height:1.85;white-space:pre-wrap">'+details+'</pre></div>';
  return '<div class="scard '+dir.toLowerCase()+'">'+
    '<div class="card-hdr"><span class="dtag '+dir+'">'+dir+'</span><span class="csym">'+( s.symbol||"–")+'</span><div class="chips">'+chips+'</div><span class="gtag '+gc+'">'+gr+'</span><span class="cts">'+( s.timestamp||"")+'</span></div>'+
    '<div class="lvl-grid">'+lvls+'</div>'+
    '<div class="cfms">'+cfms+'</div>'+
    '<div class="srow"><span class="slbl">Score</span><div class="strack"><div class="sfill" style="width:'+sc+'%;background:'+sc_color+'"></div></div><span class="snum" style="color:'+sc_color+'">'+sc+'/100</span></div>'+
    det_html+
    '</div>';
}
...
window.renderSigs=function(){
  ...
  var f=allSigs.filter(function(s){
    ...
    if(gF==="A+"&&s.grade!=="A+")return false;
    if(gF==="A"&&s.grade!=="A")return false;
    if(gF==="B"&&["C","D"].includes(s.grade))return false;
    return true;
  });
  var list=$("slist");
  if(!f.length){
...
window.toggleScanner=async function(){
  try{
    ...
    toast(d.enabled?"▶ Scanner resumed! 🚀":"⏹ Scanner stopped!",d.enabled?"buy":"sell");
    await fetchState();
  }catch(e){toast("❌ Toggle error: "+e,"");}
};
window.sw=function(tab,btn){
  activeTab=tab;
  document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));
  btn.classList.add("active");
  var tabs=["signals","trades","monitor","history","trade-cfg","log"];
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState()];if(tick%2===0)ps.push(fetchPrices());if(activeTab==="log"&&tick%2===0)ps.push(fetchLog());if(activeTab==="monitor"&&tick%2===0)ps.push(fetchMonitor());if(activeTab==="trades"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="history"&&tick%5===0)ps.push(fetchHistory());await Promise.all(ps);setTimeout(poll,3000);}
...
def api_monitor():
    """Return pairs being monitored in manipulation phase."""
    with manip_lock:
        return jsonify(list(manip_monitor.values()))

@app.route("/api/recent-trades")
def api_recent_trades():
    """Return last 10 completed trades."""
    return jsonify(list(recent_trades))

@app.route("/api/pnl")
def api_pnl():
    """Get live PnL for all open positions."""
...
def start_scanner():
    ...
    m=threading.Thread(target=manip_monitor_loop,daemon=True); m.start()
    log("Scanner + manipulation monitor threads launched.")
