...
def scanner_loop():
    ...
    while True:
        try:
            ...
            log(f"📊 GATE STATS: neutral={diag.get('neutral',0)} no_cont={diag.get('not_continuous',0)} "
                f"no_obs={diag.get('no_obs',0)} not_key={diag.get('not_at_key',0)} not_zone={diag.get('not_in_zone',0)} "
                f"no_liq={diag.get('no_liq',0)} not_tap={diag.get('not_tapping',0)} no_crt={diag.get('no_crts',0)} "
                f"no_tbs={diag.get('no_tbs',0)} rr_low={diag.get('rr_low',0)}")
...
if __name__=="__main__":
    ...
    t=threading.Thread(target=_delayed_start,daemon=True); t.start()
