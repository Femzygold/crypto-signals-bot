...
def _sweep_of_first_touch(candles, first_extreme, direction, touch_idx, lookback=40):
    """
    After the first touch, look for a candle that sweeps BEYOND first_extreme.
    SELL: sweep = candle high > first_extreme (sweeps above the first-touch high)
    BUY:  sweep = candle low  < first_extreme (sweeps below the first-touch low)
    ...
    for j, c in enumerate(search):
        ...
        if direction == "BUY"  and c["low"]  < first_extreme:
            return c, touch_idx + 1 + j
    return None, None
