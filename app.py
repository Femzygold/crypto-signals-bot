...
def test_api_connection():
    ...
    """
    # ── LOG OUTBOUND IP (needed for MEXC API whitelist) ───────────────
    try:
        my_ip = requests.get("https://api.ipify.org", timeout=5).text.strip()
        log(f"🌐 Railway outbound IP: {my_ip}  ← Copy this into MEXC API whitelist")
    except Exception as _e:
        log(f"⚠️ Could not detect outbound IP: {_e}")
  ...
  <div class="tc-wrap">
    <div class="tc-card" id="tc-card">
      ...
      <div class="tc-glass">
        ...
        <div id="tc-exit-row" style="display:none;background:rgba(239,68,68,.07);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:7px 12px;margin-bottom:10px;display:none">
