...
def mexc_sign(api_key, timestamp_ms, query_string, secret):
    """
    MEXC Futures API v1 signature:
    sign = HMAC_SHA256(api_key + timestamp + query_string, secret_key)
    """
...
def mexc_request(method, path, params=None, signed=True):
    ...
    # Build query string for GET or body string for POST
    if method == "GET":
    ...
    try:
        ...
        data = r.json()
        # MEXC returns code=0 for success
        if data.get("success") or str(data.get("code","")) == "0":
            return data.get("data"), None
        return None, data.get("message", data.get("msg", f"code={data.get('code')}"))
    except Exception as e:
        return None, str(e)
...
<div class="tc-modal" id="tc-modal">
  <div class="tc-wrap">
    <div class="tc-card" id="tc-card">
      ...
      <div class="tc-glass">
        <div class="tc-header">
          <div class="tc-brand">SIGNAL CORE BOT</div>
          <div class="tc-tagline">AI TRADER · SMARTER SIGNALS · BETTER TRADES</div>
        </div>
...
window.copyShareCard = function() {
  ...
  if (!t) return;
  var dir = t.direction === 'BUY' ? 'LONG' : 'SHORT';
  var text = 'Mad Man Strategy Scanner
'
    + '========================
'
    + (t.direction === 'BUY' ? '🟢 ' : '🔴 ') + dir + ' ' + t.symbol + '
'
    + 'Entry:  ' + fmt(t.entry) + '
'
    + 'SL:     ' + fmt(t.sl) + '
...
';
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(function() {
      toast('📋 Copied to clipboard!', 'trade');
    });
  }
};
