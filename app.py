...
def place_paper_order(sig):
    ...
    with paper_lock:
        ...
        if risk_amount < 0.1:
            risk_amount = 0.1   # floor to minimum — paper trading always allowed
        sl_distance = abs(entry - sl)
        if sl_distance <= 0:
  ...
  <div id="tab-trade-cfg" style="display:none">
    <div class="panel">
      ...
      <div class="trade-form">
        ...
        <div class="tf-group"><div class="tf-lbl">Risk per Trade (%)</div><input class="tf-inp" type="number" id="t-risk" value="1" min="0.1" max="5" step="0.1" oninput="checkRiskWarning()"/></div>
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();if(d.api_key)$("t-apikey").value=d.api_key;$("t-risk").value=d.risk_pct||1;updateTradeBadge(d.enabled);checkRiskWarning();}catch{}}
