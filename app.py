import os, json, time, secrets, requests, threading, hashlib, hmac as _hmac
...
# ── DATABASE (SQLite — auto-upgrades to PostgreSQL via DATABASE_URL) ──
import sqlite3, urllib.parse as _urlparse

DATABASE_URL = os.environ.get("DATABASE_URL", "")

def get_db():
    """Return a DB connection. Uses PostgreSQL if DATABASE_URL is set, else SQLite."""
    if DATABASE_URL:
        try:
            import psycopg2
            ...
            print(f"[DB] PostgreSQL failed: {e} — falling back to SQLite", flush=True)
    conn = sqlite3.connect("madman.db", check_same_thread=False)
...
def init_db():
    ...
    if dialect == "pg":
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                mexc_api_key TEXT DEFAULT '',
                mexc_api_secret TEXT DEFAULT '',
                trade_enabled INTEGER DEFAULT 0,
                risk_pct REAL DEFAULT 1.0,
                max_trades INTEGER DEFAULT 3,
                leverage INTEGER DEFAULT 10,
                plan TEXT DEFAULT 'free',
    ...
    conn.commit()
    conn.close()
    print("[DB] Database initialised", flush=True)
...
def db_get_user_by_id(uid):
    conn, _ = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE id = %s" if DATABASE_URL else
...
def db_create_user(email, password):
    ...
    try:
        cur.execute(
            "INSERT INTO users (email, password_hash) VALUES (%s, %s)" if DATABASE_URL else
...
def db_save_user_keys(uid, api_key, api_secret, risk_pct, max_trades, leverage, enabled):
    conn, _ = get_db()
    cur = conn.cursor()
    ph = DATABASE_URL
    cur.execute(
        """UPDATE users SET mexc_api_key=%s, mexc_api_secret=%s,
           risk_pct=%s, max_trades=%s, leverage=%s, trade_enabled=%s WHERE id=%s"""
        if ph else
        """UPDATE users SET mexc_api_key=?, mexc_api_secret=?,
           risk_pct=?, max_trades=?, leverage=?, trade_enabled=? WHERE id=?""",
        (api_key, api_secret, risk_pct, max_trades, leverage, enabled, uid)
    )
    conn.commit()
    conn.close()
...
def get_current_user():
    """Return user dict for the logged-in session, or None."""
...
# Global trade_config still used by scanner; per-user config loaded per-request
trade_config = {
    "enabled":    False,
...
def scanner_loop():
    ...
    while True:
        try:
            ...
            log("⏸ Cycle rest — 5 min before next scan round...")
            for _ in range(30):          # 30 × 10s = 300s, checks enabled flag
                if not scan_state["enabled"]: break
                time.sleep(10)
...
<link href="https://fonts.googleapis.com/css2?family=Fredoka+One&family=Nunito:wght@400;700;800;900&family=JetBrains+Mono:wght@400;700&display=swap" rel="stylesheet"/>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Nunito',sans-serif;background:#0f0e1a;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
.blob{position:fixed;border-radius:50%;filter:blur(70px);opacity:.18;z-index:0}
.b1{width:380px;height:380px;background:#7c3aed;top:-120px;left:-80px}
.b2{width:300px;height:300px;background:#db2777;bottom:-80px;right:-60px}
.b3{width:200px;height:200px;background:#0ea5e9;top:40%;left:40%}
.card{position:relative;z-index:10;background:rgba(20,18,40,.95);border:2px solid rgba(124,58,237,.4);border-radius:28px;padding:44px 38px 36px;width:100%;max-width:440px;backdrop-filter:blur(24px);box-shadow:0 40px 80px rgba(0,0,0,.7)}
.head{text-align:center;margin-bottom:28px}
.rocket{font-size:3rem;display:block;margin-bottom:10px}
.title{font-family:'Fredoka One',sans-serif;font-size:2rem;background:linear-gradient(135deg,#a78bfa,#f472b6,#38bdf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px}
.tabs{display:flex;gap:0;margin-bottom:24px;border-radius:12px;overflow:hidden;border:2px solid rgba(124,58,237,.25)}
.tab-btn{flex:1;padding:10px;background:transparent;color:rgba(167,139,250,.6);border:none;font-family:'Nunito',sans-serif;font-weight:800;font-size:.85rem;cursor:pointer;transition:all .2s}
.tab-btn.active{background:rgba(124,58,237,.25);color:#c4b5fd}
.lbl{font-size:.7rem;font-weight:800;color:rgba(167,139,250,.7);letter-spacing:.1em;text-transform:uppercase;margin-bottom:6px;display:block}
.inp{width:100%;padding:12px 16px;background:rgba(255,255,255,.05);border:2px solid rgba(124,58,237,.25);border-radius:12px;color:#e2e8f0;font-size:.95rem;font-family:'Nunito',sans-serif;font-weight:700;outline:none;transition:all .2s;margin-bottom:14px}
.inp:focus{border-color:rgba(167,139,250,.6);box-shadow:0 0 0 4px rgba(124,58,237,.1)}
.btn{width:100%;padding:13px;background:linear-gradient(135deg,#7c3aed,#db2777);color:#fff;border:none;border-radius:12px;font-family:'Fredoka One',sans-serif;font-size:1.1rem;cursor:pointer;transition:all .25s;box-shadow:0 6px 24px rgba(124,58,237,.4)}
.btn:hover{transform:translateY(-2px);box-shadow:0 10px 32px rgba(124,58,237,.55)}
.msg{border-radius:10px;padding:10px 14px;font-size:.82rem;font-weight:700;margin-bottom:14px;display:none}
.msg.err{background:rgba(239,68,68,.1);border:2px solid rgba(239,68,68,.3);color:#f87171}
.msg.ok{background:rgba(16,185,129,.1);border:2px solid rgba(16,185,129,.3);color:#34d399}
.msg.show{display:block}
.switch{text-align:center;margin-top:16px;font-size:.8rem;color:rgba(167,139,250,.6)}
.switch a{color:#a78bfa;cursor:pointer;font-weight:800;text-decoration:none}
</style>
</head>
<body>
...
<div class="card">
  ...
  <div id="form-login">
    ...
    <input class="inp" type="email" id="l-email" placeholder="your@email.com" autocomplete="email"/>
    <label class="lbl">Password</label>
    <input class="inp" type="password" id="l-pw" placeholder="Your password" autocomplete="current-password"/>
    <button class="btn" id="l-btn" onclick="doLogin()">Login 🚀</button>
    <div class="switch">No account? <a onclick="showTab('reg')">Register free</a></div>
  </div>

  <!-- REGISTER FORM -->
  <div id="form-reg" style="display:none">
    <label class="lbl">Email</label>
    <input class="inp" type="email" id="r-email" placeholder="your@email.com" autocomplete="email"/>
    <label class="lbl">Password (min 6 chars)</label>
    <input class="inp" type="password" id="r-pw" placeholder="Choose a password" autocomplete="new-password"/>
    <button class="btn" id="r-btn" onclick="doRegister()">✨ Create Account</button>
    <div class="switch">Have an account? <a onclick="showTab('login')">Login</a></div>
  </div>
</div>
<script>
function showTab(t){
...
function showMsg(text,type){const m=document.getElementById('msg');m.textContent=text;m.className='msg'+(type?' '+type+' show':'');}
async function doLogin(){
  const email=document.getElementById('l-email').value.trim();
  const pw=document.getElementById('l-pw').value.trim();
  if(!email||!pw){showMsg('Email and password required','err');return;}
  const btn=document.getElementById('l-btn');btn.disabled=true;btn.textContent='Logging in...';
  try{
    ...
    if(d.ok){showMsg('✅ Welcome back!','ok');setTimeout(()=>location.href='/dashboard',400);}
    else{showMsg('❌ '+(d.error||'Invalid credentials'),'err');btn.disabled=false;btn.textContent='Login 🚀';}
  }catch{showMsg('⚠️ Connection error','err');btn.disabled=false;btn.textContent='Login 🚀';}
}
async function doRegister(){
  const email=document.getElementById('r-email').value.trim();
  const pw=document.getElementById('r-pw').value.trim();
  if(!email||!pw){showMsg('All fields required','err');return;}
  const btn=document.getElementById('r-btn');btn.disabled=true;btn.textContent='Creating account...';
  try{
    const r=await fetch('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pw})});
    const d=await r.json();
    if(d.ok){showMsg('✅ Account created! Loading...','ok');setTimeout(()=>location.href='/dashboard',500);}
    else{showMsg('❌ '+(d.error||'Registration failed'),'err');btn.disabled=false;btn.textContent='✨ Create Account';}
  }catch{showMsg('⚠️ Connection error','err');btn.disabled=false;btn.textContent='✨ Create Account';}
}
document.addEventListener('keydown',e=>{if(e.key==='Enter'){const t=document.getElementById('form-reg').style.display==='none'?'login':'reg';t==='login'?doLogin():doRegister();}});
</script>
</body>
</html>"""
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();if(d.api_key)$("t-apikey").value=d.api_key;$("t-risk").value=d.risk_pct||1;updateTradeBadge(d.enabled);const sf=$("t-secret");sf.value="";sf.placeholder=d.secret_saved?"✅ Secret saved — leave blank to keep":"Enter your MEXC secret key";}catch{}}
...
window.saveTradeConfig=async function(){const cfg={api_key:$("t-apikey").value.trim(),risk_pct:parseFloat($("t-risk").value)||1};const sec=$("t-secret").value.trim();if(sec)cfg.api_secret=sec;if(!cfg.api_key){showTradeMsg("❌ API key required",false);return;}try{const r=await fetch("/api/trade-config",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(cfg)});const d=await r.json();if(d.ok){showTradeMsg("✅ Keys saved to your account!",true);toast("💾 Saved!","trade");$("t-secret").value="";$("t-secret").placeholder="✅ Secret saved — leave blank to keep";}else showTradeMsg("❌ Save failed",false);}catch{showTradeMsg("❌ Error",false);}};
...
def api_register():
    ...
    ok, msg = db_create_user(email, password)
    if not ok:
    ...
    resp.set_cookie("session", token, max_age=86400*30, httponly=True, samesite="Lax")
    return resp
...
def api_me():
    ...
    return jsonify({
        ...
        "has_api_key": bool(user.get("mexc_api_key", "")),
        "has_secret":  bool(user.get("mexc_api_secret", "")),
    })
...
def api_trade_config():
    ...
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        api_key   = data.get("api_key",   user.get("mexc_api_key", "")).strip()
        # Only update secret if user typed a new one
        api_secret = str(data.get("api_secret", "")).strip()
        if not api_secret:
            api_secret = user.get("mexc_api_secret", "")
        risk_pct   = float(data.get("risk_pct",   user.get("risk_pct",   1.0)))
        max_trades = int(  data.get("max_trades",  user.get("max_trades", 3)))
        leverage   = int(  data.get("leverage",    user.get("leverage",  10)))
        enabled    = bool( data.get("enabled",     user.get("trade_enabled", False)))
        db_save_user_keys(user["id"], api_key, api_secret, risk_pct, max_trades, leverage, enabled)
        # Update global trade_config so scanner uses this user's keys
        with trade_lock:
            trade_config["api_key"]    = api_key
            trade_config["api_secret"] = api_secret
            trade_config["risk_pct"]   = risk_pct
            trade_config["max_trades"] = max_trades
            trade_config["leverage"]   = leverage
            trade_config["enabled"]    = enabled
        log(f"⚙️ {user['email']} trade config updated. Auto-trade: {'ON' if enabled else 'OFF'}")
        return jsonify({"ok": True})

    # GET — return config for this user, never expose the real secret
    return jsonify({
        "api_key":     user.get("mexc_api_key", ""),
        "secret_saved": bool(user.get("mexc_api_secret", "").strip()),
        "risk_pct":    user.get("risk_pct",   1.0),
        "max_trades":  user.get("max_trades", 3),
        "leverage":    user.get("leverage",  10),
        "enabled":     bool(user.get("trade_enabled", False)),
    })
