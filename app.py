...
def mexc_sign(api_key, timestamp_ms, query_string, secret):
    """
    MEXC Futures API v1 correct signature format:
    sign = HMAC_SHA256(timestamp + api_key + queryString, secret_key)
    Note: order is timestamp FIRST, then apiKey, then params
    """
    raw = str(timestamp_ms) + str(api_key) + str(query_string)
