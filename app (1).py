...
settings_config = {
    ...
    "mexc_api_key": os.environ.get("MEXC_API_KEY", "mx0vglgGjqnoPDiTFu"),
    "mexc_api_secret": os.environ.get("MEXC_API_SECRET_KEY", "e13578211318499baa3852677365d3cb"),
    "live_price_update_interval": 2,
    "scan_interval": 60,
    "cycle_rest_interval": 5,
...
def scanner_loop():
    ...
    while True:
        try:
            ...
            for _ in range(5):          # 5 × 1s = 5s, checks enabled flag
                if not scan_state["enabled"]: break
                time.sleep(1)
...
<div class="tab-wrap">
  ...
  <div id="tab-settings" style="display:none">
    <!-- API KEYS & SECRETS -->
    <div class="panel">
      <div class="panel-ttl">🔍 API Keys & Secrets</div>
      <div class="info-box info-blue">ℹ️ <b>Security:</b> API keys are stored securely and never exposed in responses. Masked with *** when displayed.</div>
      <div class="trade-form">
        <div class="tf-group"><div class="tf-lbl">MEXC API Key</div><input class="tf-inp" type="text" id="s-mexc-key" placeholder="Your MEXC API key"/></div>
        <div class="tf-group"><div class="tf-lbl">MEXC Secret Key</div><input class="tf-inp" type="password" id="s-mexc-secret" placeholder="Your MEXC secret key"/></div>
      </div>
    </div>
    <!-- TELEGRAM NOTIFICATIONS -->
    <div class="panel">
      <div class="panel-ttl">📢 Signal & Trade Notifications</div>
      <div class="info-box info-green">📱 <b>Telegram Bot:</b> Receive instant notifications when signals are found and trades are placed. Get your bot token from @BotFather on Telegram.</div>
      <div class="trade-form">
        <div class="tf-group"><div class="tf-lbl">Telegram Bot Token</div><input class="tf-inp" type="password" id="s-tg-token" placeholder="Your Telegram bot token"/></div>
        <div class="tf-group"><div class="tf-lbl">Telegram Chat ID</div><input class="tf-inp" type="text" id="s-tg-chat" placeholder="Your Telegram chat ID"/></div>
        <div style="display:flex;gap:12px;margin-top:12px">
          <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer;background:rgba(16,185,129,.08);border:1.5px solid rgba(16,185,129,.22);border-radius:10px;padding:10px 14px">
            <input type="checkbox" id="s-signal-notif" checked style="width:18px;height:18px;cursor:pointer"/>
            <span style="color:var(--text);font-weight:600">Signal Notifications</span>
          </label>
          <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer;background:rgba(16,185,129,.08);border:1.5px solid rgba(16,185,129,.22);border-radius:10px;padding:10px 14px">
            <input type="checkbox" id="s-trade-notif" checked style="width:18px;height:18px;cursor:pointer"/>
            <span style="color:var(--text);font-weight:600">Trade Notifications</span>
          </label>
        </div>
      </div>
    </div>
    <!-- UPDATE INTERVALS -->
    <div class="panel">
      <div class="panel-ttl">⏱️ Update Intervals (Seconds)</div>
      <div class="info-box info-yellow">⚡ <b>Performance Tuning:</b> Lower intervals = more frequent updates but higher CPU usage. Recommended: Price 2s, Scan 60s, Cycle Rest 5s.</div>
      <div class="trade-form">
        <div class="tf-group">
          <div class="tf-lbl">Live Price Update Interval</div>
          <div style="display:flex;align-items:center;gap:8px">
            <input class="tf-inp" type="number" id="s-price-interval" min="1" max="60" step="1" style="flex:1"/>
            <span style="color:var(--dim);font-family:'JetBrains Mono',monospace;font-size:.75rem;min-width:40px">seconds</span>
          </div>
        </div>
        <div class="tf-group">
          <div class="tf-lbl">Scan Interval (between pairs)</div>
          <div style="display:flex;align-items:center;gap:8px">
            <input class="tf-inp" type="number" id="s-scan-interval" min="1" max="300" step="1" style="flex:1"/>
            <span style="color:var(--dim);font-family:'JetBrains Mono',monospace;font-size:.75rem;min-width:40px">seconds</span>
          </div>
        </div>
        <div class="tf-group">
          <div class="tf-lbl">Cycle Rest Interval (between scans)</div>
          <div style="display:flex;align-items:center;gap:8px">
            <input class="tf-inp" type="number" id="s-cycle-interval" min="1" max="300" step="1" style="flex:1"/>
            <span style="color:var(--dim);font-family:'JetBrains Mono',monospace;font-size:.75rem;min-width:40px">seconds</span>
          </div>
        </div>
      </div>
    ...
    <div class="panel">
      <div class="trade-actions">
        <button class="trade-btn tb-save" onclick="saveSettings()">💾 Save All Settings</button>
        <button class="trade-btn tb-chk" onclick="loadSettings()">🔄 Reload Settings</button>
      </div>
      <div class="t-status" id="settings-msg"></div>
    </div>
  </div>
</div>
...
/* ─── SETTINGS ─────────────────────────── */
async function loadSettings(){try{const r=await fetch("/api/settings");const d=await r.json();$("s-mexc-key").value=d.mexc_api_key||"";$("s-mexc-secret").value=d.mexc_api_secret||"";$("s-tg-token").value=d.telegram_bot_token||"";$("s-tg-chat").value=d.telegram_chat_id||"";$("s-price-interval").value=d.live_price_update_interval||2;$("s-scan-interval").value=d.scan_interval||60;$("s-cycle-interval").value=d.cycle_rest_interval||5;$("s-signal-notif").checked=d.signal_notifications_enabled!==false;$("s-trade-notif").checked=d.trade_notifications_enabled!==false;}catch{}}
function showSettingsMsg(msg,ok){const el=$("settings-msg");el.textContent=msg;el.className="t-status "+(ok?"ok":"err");setTimeout(()=>el.className="t-status",4000);}
window.saveSettings=async function(){const cfg={mexc_api_key:$("s-mexc-key").value.trim(),mexc_api_secret:$("s-mexc-secret").value.trim(),telegram_bot_token:$("s-tg-token").value.trim(),telegram_chat_id:$("s-tg-chat").value.trim(),live_price_update_interval:parseInt($("s-price-interval").value)||2,scan_interval:parseInt($("s-scan-interval").value)||60,cycle_rest_interval:parseInt($("s-cycle-interval").value)||5,signal_notifications_enabled:$("s-signal-notif").checked,trade_notifications_enabled:$("s-trade-notif").checked};try{const r=await fetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(cfg)});const d=await r.json();if(d.ok){showSettingsMsg("✅ All settings saved!",true);toast("⚙️ Settings saved","trade");}else showSettingsMsg("❌ Save failed",false);}catch{showSettingsMsg("❌ Error saving settings",false);}};
...
window.saveTcCard = function() {
  ...
  if (typeof html2canvas !== 'undefined') {
    html2canvas(card, {
      ...
      var filename = 'madman-' + sym.replace('/','') + '-' + Date.now() + '.png';
      var dataUrl = canvas.toDataURL('image/png');
      
      // Try to save to gallery on mobile using Web Share API
      if (navigator.share && navigator.canShare) {
        canvas.toBlob(function(blob) {
          ...
          if (navigator.canShare({ files: [file] })) {
            navigator.share({
              files: [file],
              title: 'Mad Man Trade Card',
              text: sym + ' Trade Card'
            }).then(function() {
              toast('📸 Card shared & saved!', 'trade');
            }).catch(function(e) {
...
def api_settings():
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        with settings_lock:
            if "telegram_bot_token" in data: settings_config["telegram_bot_token"] = data["telegram_bot_token"]
            if "telegram_chat_id" in data: settings_config["telegram_chat_id"] = data["telegram_chat_id"]
            if "mexc_api_key" in data: settings_config["mexc_api_key"] = data["mexc_api_key"]
            if "mexc_api_secret" in data: settings_config["mexc_api_secret"] = data["mexc_api_secret"]
            if "live_price_update_interval" in data: settings_config["live_price_update_interval"] = int(data["live_price_update_interval"])
            if "scan_interval" in data: settings_config["scan_interval"] = int(data["scan_interval"])
            if "cycle_rest_interval" in data: settings_config["cycle_rest_interval"] = int(data["cycle_rest_interval"])
            if "signal_notifications_enabled" in data: settings_config["signal_notifications_enabled"] = bool(data["signal_notifications_enabled"])
            if "trade_notifications_enabled" in data: settings_config["trade_notifications_enabled"] = bool(data["trade_notifications_enabled"])
        log(f"⚙️ Settings updated")
        return jsonify({"ok": True, "settings": {k:("***" if k in ["mexc_api_secret","telegram_bot_token"] and v else v) for k,v in settings_config.items()}})
    with settings_lock:
        cfg = {k: ("***" if k in ["mexc_api_secret","telegram_bot_token"] and v else v) for k,v in settings_config.items()}
    return jsonify(cfg)
